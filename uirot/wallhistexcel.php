...
$objPHPExcel = new PHPExcel();
	...
	if($Walltype == "ALL"){
			...
			if($partyType == "ALL"){
				$query = "select a.balance_history_id,a.date_time,if(a.wallet_type='C','C - Commission Wallet',if(a.wallet_type='M','M - Main Wallet',if(a.wallet_type='O','O-Other Wallet','-'))) as wallet_type,if(a.party_type='A','A-Agent',if(a.party_type='C','C-Champion',if(a.party_type='P','P-Personal',if(a.party_type='S','S-Sub-Agent',if(a.party_type='O','O-Others','-'))))) as party_type,if(a.party_type='A',(select concat(agent_code,'-',agent_name) from agent_info where agent_code=a.party_code),(select concat(champion_code,'-',champion_name) from champion_info where champion_code=a.party_code)) as party_code,a.available_balance,a.current_balance,ifNULL(a.credit_limit,'-') as credit_limit,ifNULL(a.daily_limit,'-') as daily_limit,a.advance_amount,a.minimum_balance,a.previous_current_balance,ifNULL(a.uncleared_balance,'-') as uncleared_balance,ifNULL(a.last_tx_no,'-') as last_tx_no,ifNULL(a.last_tx_amount,'-') as last_tx_amount,ifNULL(a.last_tx_date,'-') as last_tx_date,if(a.active='Y','Y-Yes',if(a.active='N','N-No','-')) as active,ifNULL(a.block_status,'-') as block_status,ifNULL(a.block_date,'-') as block_date,ifNULL(a.block_reason_id,'-') as block_reason_id FROM  wallet_balance_history WHERE wallet_type='$Walltype' and date(date_time) between '$startDate' and '$endDate'  group by wallet_type,party_type,party_code,a.balance_history_id order by date_time";
			}
			else  if($partyCode !== "ALL" ){ 
