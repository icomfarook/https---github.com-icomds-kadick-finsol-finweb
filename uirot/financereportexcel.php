				...
				if($typeDetail == false) {
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							$heading = array("Date","Request Amount", " State");
								$headcount = 3;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount,concat(c.name) as State  FROM fin_service_order a,agent_info b,state_list c WHERE a.user_id= b.user_id and b.state_id = c.state_id and date(a.date_time) between '$startDate' and '$endDate' group by  State  ,b.agent_code,b.agent_name, date(a.date_time) order by date(a.date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount", " State");
								$headcount = 3;
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount ,concat(c.name) as State  FROM fin_service_order a,agent_info b,state_list c WHERE a.user_id= b.user_id and b.state_id = c.state_id and date(a.date_time) between '$startDate' and '$endDate' group by  State  ,b.agent_code,b.agent_name date(a.date_time) order by date(a.date_time)";
							}
							if($ba == 'bo') {
							$heading = array("Date","Request Amount","Total Amount", " State");
								$headcount = 4;
								$query ="SELECT date(a.date_time) as Date,  SUM(a.request_amount) as request_amount, SUM(a.total_amount) as total_amount, b.agent_code,b.agent_name ,concat(c.name) as State  FROM fin_service_order a,agent_info b,state_list c WHERE a.user_id= b.user_id and b.state_id = c.state_id and date(a.date_time) between '$startDate' and '$endDate'   group by  State,b.agent_code,b.agent_name , date(a.date_time) order by date(a.date_time)";
							}
						}
						else {
							if($ba == 'ra'){
							$heading = array("Date","Request Amount","Agent Name", " State");
								$headcount = 4;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent, concat(c.name) as State  FROM fin_service_order a, agent_info b,state_list c WHERE b.state_id=c.state_id and  a.user_id = b.user_id and date(date_time) between '$startDate' and '$endDate' group by  State,b.parent_code, agent, date(a.date_time) order by  agent, date(a.date_time)";
							}
							if($ba == 'ta'){
							$heading = array("Date","Total Amount","Agent Name", " State");
								$headcount = 4;
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM fin_service_order a, agent_info b,state_list c WHERE b.state_id= c.state_id and a.user_id = b.user_id and date(date_time) between '$startDate' and '$endDate' group by  State,b.parent_code  , agent, date(a.date_time) order by  agent, date(a.date_time)";
							}
							if($ba == 'bo'){
							$heading = array("Date","Request Amount", "Total Amount","Agent Name", " State");
								$headcount = 5;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount, SUM(a.total_amount) as total_amount, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM fin_service_order a, agent_info b ,state_list c WHERE b.state_id= c.state_id and  a.user_id = b.user_id and date(date_time) between '$startDate' and '$endDate' group by  State,b.parent_code  , agent, date(a.date_time) order by  agent, date(a.date_time)";
							}
						}
					}
					else {
						if($agentDetail == false) {
							...
							$heading = array("Date","Request Amount", " State");
								$headcount = 3;
								$query ="SELECT date(a.date_time) as Date,  SUM(a.request_amount) as request_amount,concat(c.name) as State FROM fin_service_order a, agent_info b, state_list c WHERE b.state_id = c.state_id and b.agent_code = '$agentName' and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate'  group by  State  , date(a.date_time) order by date(a.date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount", " State");
								$headcount = 3;
							...
							$heading = array("Date","Request Amount","Total Amount", " State");
								$headcount = 4;
								$query ="SELECT date(a.date_time) as Date,  SUM(a.request_amount) as request_amount, SUM(a.total_amount) as total_amount,concat(c.name) as State FROM fin_service_order a, agent_info b,state_list c WHERE b.state_id = c.state_id and  b.agent_code = '$agentName' and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate' group by  State  , date(a.date_time) order by date(a.date_time)";
							}
						}
						else {
							if($ba == 'ra') {
					...
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							if($ba == 'bo') {
							$heading = array("Date","Request Amount","Total Amount","Order Type", " State");
								$headcount = 5;
							...
							if($ba == 'ra') {
							$heading = array("Date","Request Amount","Order Type","Agent Name", " State");
								$headcount = 5;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount, a.service_feature_code as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM fin_service_order a, agent_info b, state_list c WHERE b.state_id = c.state_id  and  date(a.date_time) between '$startDate' and '$endDate' and a.user_id = b.user_id group by  State,b.parent_code  , a.service_feature_code, agent, date(a.date_time) order by  a.service_feature_code, agent, date(a.date_time)";
							}
							if($ba == 'ta') {
							...
							if($ba == 'bo') {
							$heading = array("Date", "Request Amount","Total Amount","Order Type","Agent Name", " State");
								$headcount = 6;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount, SUM(a.total_amount) as total_amount, a.service_feature_code as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM fin_service_order a, agent_info b,state_list c WHERE b.state_id = c.state_id  and date(a.date_time) between '$startDate' and '$endDate' and a.user_id = b.user_id group by  State,b.parent_code  , a.service_feature_code, agent, date(a.date_time) order by  a.service_feature_code, agent, date(a.date_time)";
							}
						}
				...
				if($typeDetail == false) {
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							$heading = array("Date","Request Amount", " State");
								$headcount = 3;
								$query ="SELECT date(date_time) as Date, SUM(request_amount) as request_amount,concat(c.name) as State FROM fin_service_order a,agent_info b,state_list c  WHERE  a.user_id = b.user_id and b.state_id = c.state_id and service_feature_code = '$type' and date(date_time) between '$startDate' and '$endDate'group by  date(date_time),State order by date(date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount", " State");
								$headcount = 3;
								$query ="SELECT date(date_time) as Date, SUM(total_amount) as total_amount,concat(c.name) as State FROM fin_service_order a,agent_info b,state_list c  WHERE  a.user_id = b.user_id and b.state_id = c.state_id and service_feature_code = '$type' and date(date_time) between '$startDate' and '$endDate'group by date(date_time),State order by date(date_time)";
							}
							if($ba == 'bo') {
							$heading = array("Date","Request Amount","Total Amount", " State");
								$headcount = 4;
								$query ="SELECT date(date_time) as Date, SUM(request_amount) as request_amount, SUM(total_amount) as total_amount,concat(c.name) as State FROM fin_service_order a,agent_info b,state_list c  WHERE  a.user_id = b.user_id and b.state_id = c.state_id and service_feature_code = '$type' and date(date_time) between '$startDate' and '$endDate'group by State,  date(date_time) order by date(date_time)";
							}
						}
						else {
							if($ba == 'ra') {
							$heading = array("Date","Request Amount","Agent Name", " State");
								$headcount = 4;
								$query ="SELECT date(a.date_time) as Date, SUM(request_amount) as request_amount , concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM fin_service_order a, agent_info b,state_list c WHERE b.state_id=c.state_id and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type' group by  State,b.parent_code  , agent, date(a.date_time) order by  agent, date(a.date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount","Agent Name", " State");
								$headcount = 4;
								$query ="SELECT date(a.date_time) as Date, SUM(total_amount) as total_amount , concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM fin_service_order a, agent_info b,state_list c WHERE b.state_id=c.state_id and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type' group by  State,b.parent_code  , agent, date(a.date_time) order by  agent, date(a.date_time)";
							}
							if($ba == 'bo') {
							$heading = array("Date","Request Amount","Total Amount","Agent Name", " State");
								$headcount = 5;
								$query ="SELECT date(a.date_time) as Date, SUM(request_amount) as request_amount, SUM(total_amount) as total_amount , concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM fin_service_order a, agent_info b ,state_list c WHERE b.state_id=c.state_id and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type' group by  State,b.parent_code  , agent, date(a.date_time) order by  agent, date(a.date_time)";
							}
						}
					}
					else {
						if($agentDetail == false) {
							...
							$heading = array("Date","Request Amount", " State");
								$headcount = 3;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount,concat(c.name) as State  FROM fin_service_order a, agent_info b,state_list c WHERE  b.state_id=c.state_id and  b.agent_code = '$agentName' and a.user_id = b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate'group by  State  , date(a.date_time) order by date(a.date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount", " State");
								$headcount = 3;
				...
				if($typeDetail == false) {
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							$heading = array("Date","Request Amount", " State");
								$headcount = 3;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount, b.agent_code,b.agent_name ,concat(c.name) as State  FROM fin_service_order a,agent_info b,state_list c WHERE a.user_id= b.user_id and b.state_id = c.state_id and date(a.date_time) between '$startDate' and '$endDate'and c.state_id=$state  group by  State,b.agent_code, b.agent_name  , date(a.date_time) order by date(a.date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount", " State");
								$headcount = 3;
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, b.agent_code,b.agent_name ,concat(c.name) as State  FROM fin_service_order a,agent_info b,state_list c WHERE a.user_id= b.user_id and b.state_id = c.state_id and date(a.date_time) between '$startDate' and '$endDate'and c.state_id=$state  group by  State  ,b.agent_code, b.agent_name, date(a.date_time) order by date(a.date_time)";
							}
							if($ba == 'bo') {
							$heading = array("Date","Request Amount","Total Amount", " State");
								$headcount = 4;
				...
				if($typeDetail == false) {
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							$heading = array("Date","Request Amount", " State");
								$headcount = 3;
								$query ="SELECT date(a.date_time) as Date, SUM(a.request_amount) as request_amount ,b.agent_code,b.agent_name ,concat(c.name) as State  FROM fin_service_order a,agent_info b,state_list c WHERE a.user_id= b.user_id and b.state_id = c.state_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate' and c.state_id='$state' group by  State  , date(a.date_time),b.agent_code,b.agent_name order by date(a.date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount", " State");
								$headcount = 3;
					...
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							$heading = array("Date","Request Amount","Order Type", " State");
								$headcount = 4;
								$query ="SELECT date(date_time) as Date, SUM(request_amount) as request_amount, service_feature_code as 'Order Type',concat(c.name) as State  FROM fin_service_order a, agent_info b,state_list c WHERE b.state_id = c.state_id and  service_feature_code = '$type' and date(date_time) between '$startDate' and '$endDate' and c.state_id='$state'  group by  State  , service_feature_code, date(date_time) order by service_feature_code,date(date_time)";
							}
							if($ba == 'ta') {
							$heading = array("Date","Total Amount","Order Type", " State");
								$headcount = 4;
