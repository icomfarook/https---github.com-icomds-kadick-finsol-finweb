...
$objPHPExcel = new PHPExcel();
		...
		if($profileid == 1 || $profileid == 10 || $profileid == 24 || $profileid == 22 || $profileid == 20 || $profileid == 23 || $profileid == 26 || $profileid  == 50) {
			$query = "SELECT a.acc_service_order_no,concat(a.service_feature_code, ' - ', d.feature_description) as service_feature_code,concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user,  a.total_amount,  concat(f.name) as bank,  a.date_time as date_time,c.update_time,c.bvn,ifNULL(c.email,'-') as email,c.account_number,ifNULL(c.account_balance,'-') as account_balance,group_concat(p.charge_value ORDER BY p.service_charge_party_name) as chargese FROM acc_service_order a, agent_info b, acc_request c, service_feature d,user_pos e,bank_master f,acc_service_order_comm p WHERE a.acc_service_order_no = c.order_no and c.status = 'S' and a.user_id = b.user_id and a.service_feature_code = d.feature_code and a.user_id = e.user_id and a.bank_id = f.bank_master_id and a.acc_service_order_no = p.acc_service_order_no";
		}
		if($profileid  == 51) {
		...
		error_log($query);
		$heading = array("Order No","Order Type","Agent","Total Amount","Bank", "Date Time","Update Time","Bvn","Email","Account Number","Account Balance","Total Splited Commission","Agent Commission","Champion Commission","Kadick Commission");
		$headcount = 15;
