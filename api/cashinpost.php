<?php
error_log("inside cashinpost.php");
$error_path = "N";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ( isset($data->countryId ) && !empty($data->countryId) && isset($data->stateId) && !empty($data->stateId) 
		...
		&& isset($data->cardname) && !empty($data->cardname) && isset($data->bankId) && !empty($data->bankId)
		...
		$tx_ref = "CI".date('y').date('z').str_pad($data->transactionId, 7,'0',STR_PAD_LEFT);
