...
if(isset($_POST['eng'])) {	
	//unset($_COOKIE["lang"]);
	setcookie("lang", "hau", time()+3600, "/","", 0);
}
if(isset($_POST['haus'])) {
	unset($_COOKIE["lang"]);	
	setcookie("lang", "eng", time()+3600, "/","", 0);
	
}
if($_COOKIE["lang"] == "eng" || $_COOKIE["lang"] =="" ) {
...
if(isset($_POST['login'])) {
	...
	if ( empty($_SESSION['captcha_code'] ) || strcasecmp($_SESSION['captcha_code'], $_POST['captcha_code']) != 0 ){  
		...
		$user_check_query = "SELECT temp_password, first_time_login, user_id, user_name, password, active, COALESCE(invalid_attempt,0) as invalid_attempt, locked, access_restrict, pos_access from user WHERE UPPER(user_name) = UPPER('".$uname."') and profile_id in (50) limit 1";
			...
			if($user_check_count > 0){
				...
				if ( $user_active == "Y" ) {
					if ( $locked != "Y" && $invalid_attempt <= $admin_attempt_limit ) {
						...
						if($successpassword) {
							if($pos_access == "N")	{
								$user_otp_query = "SELECT b.otp_dynamic, b.otp_value, b.key_string, b.pin, b.pin_flag FROM user a, user_otp b WHERE a.user_id = b.user_id and UPPER(a.user_name) = UPPER('".$uname."') and a.user_id = '".$user_id."' and a.active = 'Y' and a.profile_id in (50) limit 1";
								...
								if(!$user_otp_result) {
									...
									if( $formatted_otp_value == $tokensplit0 ){
										...
										if ( $pin_flag == "N" || ( $pin_flag == "Y" && $pin_sort == $tokensplit1 ) ) {
											...
											if($first_time_login == "N") {
												$user_sec_check_query = "SELECT a.user_id, a.user_name, a.first_name, a.last_name, a.active, a.last_login, a.invalid_attempt, a.profile_id, a.email, a.locked, b.profile_name,c.champion_code,c.champion_name,c.local_govt_id,c.state_id,c.party_category_type_id, b.auth_id, c.language_id, c.country_id from user a, profile b, champion_info c WHERE a.profile_id = b.profile_id and a.user_name = '".$uname."' and c.user_id = a.user_id and a.profile_id in (50) limit 1";
												...
												if ( $user_sec_check_result ) {
													...
													if($user_sec_check_count > 0) {
														...
														if($login_allow == "Y") {
															...
															$service_group_query = "SELECT distinct a.service_group_id, a.service_group_name FROM service_group a, service_feature_menu b, user_service_type c WHERE a.service_group_id = b.service_group_id and b.profile_id = $profile_id and a.active = 'Y' and b.active = 'Y' and c.active = 'Y' and c.service_group_id = b.service_group_id  and c.party_type = 'C' and c.party_code = '$champion_code' and c.active = 'Y' order by a.priority";
															...
															if ( $service_group_result ) {
																while($service_group_row = mysqli_fetch_assoc($service_group_result)) {
																	...
																	$service_feature_query = "select c.service_feature_id, c.feature_description, c.href from service_group a,service_feature_menu b, service_feature c where a.service_group_id = ".$service_group_row['service_group_id']." and a.service_group_id = b.service_group_id and b.profile_id = ".$profile_id." and b.service_feature_id = c.service_feature_id and c.active = 'Y' and b.active = 'Y' and a.active= 'Y' order by b.priority"; 
															...
															$_SESSION['party_code'] = $champion_code;
															$_SESSION['party_type'] = 'C';
															$_SESSION['party_name'] = $champion_name;
		...
		<a href="login.php"><img src="../common/images/km_logo.png" height="42" width="100" style="float: right;"></a></h2>
            <div  style='text-align:center' class="loading-spiner-holder" data-loading1 ><div class="loading-spiner"><img style='width:100%' align="middle" src="../common/img/loading-48.gif" /></div></div>
		     <form onsubmit="return validateForm()" class="form-signin" name='adminrloginform' method='POST' action="">            <div class=" w3l-form-group">
                <label><?php echo CHAMP_USERNAME; ?></label>
                <div class="group">
            ...
            <div class=" w3l-form-group">
                <label><?php echo CHAMP_PASSWORD; ?></label>
                <div class="group">
			...
			<div class=" w3l-form-group">
                <label><?php echo CHAMP_TOKEN; ?></label>
                <div class="group">
                    <i class="fas fa-unlock"></i>
                    <input type="password" class="form-control" name='token' maxlength="10" Placeholder="Token" >
                </div>
            </div>
			<div class=" w3l-form-group">
                <label><?php echo CHAMP_CAPTCHA; ?></label>
                <div class="group">
                    <i class="fas fa-unlock"></i>
			...
			<div class=" w3l-form-group">
                <p ><img style='' src="../common/captcha.php?rand=<?php echo rand();?>" id='captchaimg'/><span style='padding-left:5px;color:white'><?php echo CHAMP_CANT_READ_CAPTCHA_CLICK; ?> <a style="color:blue;font-size:16px; " href='javascript: refreshCaptcha();'><?php echo CHAMP_HERE; ?></a></span></p>
				<p style="color:red;font-size:16px;"><?php if(isset($_POST['login'])) { echo $msg;} ?></p>
				<button name='login' class="btn btn-lg btn-primary btn-block btn-signin" type="submit"><?php echo CHAMP_SIGNIN; ?></button>
				<br /><br />
				<button class="btn btn-lg btn-primary btn-block btn-signin" type="reset"><?php echo CHAMP_RESET; ?></button>
				<br />			
            </div>            
         </form>
		<form class="form-signin" action="" name='' method="POST">		 
		 <?php if($_COOKIE["lang"] == "eng" || $_COOKIE["lang"] =="" ) {?>
					<button name='eng' style='padding: 0px;font-size: 13px;background-color: transparent;text-align: left;text-transform: none;text-decoration: underline;' type="submit">Yaren hausa</button>
				<?php } if($_COOKIE["lang"] == "hau") {?>
				    <button name='haus' style='padding: 0px;font-size: 13px;background-color: transparent;text-align: left;text-transform: none;text-decoration: underline;' type="submit">English Language</button>
				<?php } ?>
		</form>			
    </div>
