<?php
	...
	error_log("inside pcposapi/recharge_data.php");
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		error_log("recharge_data <== ".json_encode($data));

		if( isset($data->operation) && $data->operation == 'RECHARGE_DATA_BUY') {
			error_log("inside operation == RECHARGE_DATA_BUY method");
            if (  isset($data->requestAmount) && !empty($data->requestAmount) && isset($data->operatorCode) && !empty($data->operatorCode)
				...
				&& isset($data->dnCode) && !empty($data->dnCode) && isset($data->dnValue) && !empty($data->dnValue)
				&& isset($data->circleId) && !empty($data->circleId) && isset($data->brandId) && !empty($data->brandId)
				&& isset($data->oprProductId) && !empty($data->oprProductId)
	  		) {
				...
				$dnCode = $data->dnCode;
				$dnValue = $data->dnValue;
				$circleId = $data->circleId;
				$brandId = $data->brandId;
				$oprProductId = $data->oprProductId;
                $amsCharge = 0;
                $partnerCharge = 0;
				$otherCharge = 0;
				...
				if ( $local_signature == $signature ) {									
					...
					if ( $validate_result != 0 ) {
						...
						$select_data_plan_query = "select operator_plan_description, dn_code, dn_value, retailer_rate from evd_master2 where operator_id = ".$operatorId." and operator_plan_id = ".$operatorPlanId." and country_id = ".$countryId." and circle_id = ".$circleId." and brand_id = ".$brandId." and product_id = ".$oprProductId." and active = 'Y'";
						error_log("select_data_plan_query = ".$select_data_plan_query);
						$select_data_plan_result = mysqli_query($con, $select_data_plan_query);
						if($select_data_plan_result) {
							$select_data_plan_count = mysqli_num_rows($select_data_plan_result);
							if($select_data_plan_count > 0) {	
								$select_data_plan_row = mysqli_fetch_assoc($select_data_plan_result);
								$db_operator_plan = $select_data_plan_row['operator_plan_description'];
								$db_dn_code = $select_data_plan_row['dn_code'];
								$db_dn_value = $select_data_plan_row['dn_value'];
								$db_retailer_rate = $select_data_plan_row['retailer_rate'];
							}else {
								error_log("No entries in evd_master2 table for Operator Plan Id = ".$operatorPlanId);
							}
						}else {
							error_log("error in reading evd_master2 table for Operator Plan Id = ".$operatorPlanId);
						}

						if ( floatval($dnValue) == floatval($db_dn_value) && floatval($dnCode) == floatval($db_dn_code) 
							&& floatval($requestedAmount) == floatval($db_retailer_rate) ) {
							...
							$response["statusCode"] = "210";
							$response["result"] = "Error";
							$response["message"] = "Failure: Invalid client request";
							$response["partnerId"] = $partnerId;
							$response["signature"] = 0;
						}
					}
		...
		}else if( isset($data->operation) && $data->operation == 'RECHARGE_DATA_LIST') {
			error_log("inside operation == RECHARGE_DATA_LIST method");
            if (  isset($data->operatorId) && !empty($data->operatorId)  && isset($data->operatorCode) && !empty($data->operatorCode)
            	&& isset($data->circleId) && !empty($data->circleId) && isset($data->countryId) && !empty($data->countryId) 
            	&& isset($data->partyCode) && !empty($data->partyCode) && isset($data->partyType) && !empty($data->partyType) 
            	&& isset($data->productId) && !empty($data->productId) && isset($data->userId) && !empty($data->userId) 
            	&& isset($data->signature) && !empty($data->signature) && isset($data->key1) && !empty($data->key1) 
				&& isset($data->brandId) && !empty($data->brandId) && isset($data->oprProductId) && !empty($data->oprProductId) 
	  		) {
	  			ini_set('max_execution_time', 60);
				set_time_limit(60);
				error_log("inside all inputs are set correctly");	
				...
				if ( $local_signature == $signature ) {
					...
					$select_data_plan_query = "select operator_id, operator_plan_id, operator_plan_description, dn_code, dn_value, retailer_rate from evd_master2 where operator_id = ".$operatorId." and country_id = ".$countryId." and circle_id = ".$circleId." and brand_id = ".$brandId." and product_id = ".$oprProductId." and active = 'Y' and operator_plan_id not in (93, 52, 77, 81, 53) order by master2_id";
					 error_log("select_data_plan_query = ".$select_data_plan_query);
					 $select_data_plan_result = mysqli_query($con, $select_data_plan_query);
					 $response["operatorPlans"] = array();
					 if ( $select_data_plan_result ) {
						 while($select_data_plan_row = mysqli_fetch_assoc($select_data_plan_result)) {
							 $operatorPlan = array();
							 $operatorPlan['operatorId'] = $select_data_plan_row['operator_id'];
							 $operatorPlan['operatorPlanId'] = $select_data_plan_row['operator_plan_id'];
							 $operatorPlan['operatorPlanDescription'] = $select_data_plan_row['operator_plan_description'];
							 $operatorPlan['dnCode'] = $select_data_plan_row['dn_code'];
							 $operatorPlan['dnValue'] = $select_data_plan_row['dn_value'];
							 $operatorPlan['amount'] = $select_data_plan_row['retailer_rate'];
							 array_push($response["operatorPlans"], $operatorPlan);
						 }
						 $response["result"] = "Success";
						 ...
						 $response["signature"] = $server_signature;
							...
							$response["result"] = "Error";
							$response["message"] = "Error in finding your data plan list";
	...
	// echoing JSON response
	error_log("recharge_data ==> ".json_encode($response));
	echo json_encode($response);
	return;			
