		...
		if(isset($data -> operation) && $data -> operation == 'REPORT_MYACCOUNT') {
			...
			if ( isset($data->signature) && !empty($data->signature) && isset($data->key1) && !empty($data->key1)
				...
				if ( $local_signature == $signature ) {
					...
					//$transaction_query = "(select a.fin_service_order_no as order_no, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)', if(a.service_feature_code = 'MP0', 'Cash-Out (Card)', if(a.service_feature_code = 'MP9', 'Fund Wallet', if(a.service_feature_code = 'COP', 'Cash-Out (Phone)', if(a.service_feature_code = 'COD', 'Cash-Out (USSD)', 'Other')))))) as order_type, if(a.service_feature_code='COP',c.account_no,IFNULL(b.name, '-')) as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, if(a.service_feature_code='COD', ifnull(c.rrn,'-'), ifnull(a.customer_name, '-')) as customer_name, if(a.service_feature_code='COD',c.account_no, a.mobile_no) as mobile_no, a.auth_code as session_id, a.reference_no, a.comment, if(c.service_feature_code='CIN',c.account_no,c.approver_comments) as approver_comments, a.date_time, '' as date_time1, c.status, c.sender_name from fin_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join fin_request c on a.fin_service_order_no = c.order_no where a.user_id = $userId ) union all (select a.e_transaction_id as order_no, if (a.service_feature_code='AFX','Recharge',if(a.service_feature_code='AFI','Fixed Recharge', 'Other')) as order_type, b.operator_description as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge+a.partner_charge) as service_charge, a.other_charge as other_charge, '-' as customer_name, a.mobile_number as mobile_no, if(a.service_feature_code='AFI',a.reference_no2, '') as session_id, a.reference_no, a.opr_plan_desc as comments, if(a.service_feature_code='AFI',a.reference_no3, '') as approver_comments, a.date_time, '' as date_time1, 'S' as status, if(a.service_feature_code='AFI', a.reference_no4, '') as sender_name from evd_transaction a, operator b where a.operator_id = b.operator_id and a.user_id = $userId ) union all (select a.bp_service_order_no as order_no, if(a.service_feature_code='PEB','Bill Payment',if(a.service_feature_code='PIT','Internet Payment', if(a.service_feature_code = 'PTV', 'Bill Payment - TV', if(a.service_feature_code='PTN','Tin Payment', if(a.service_feature_code='PFR','FRSC/Imm Payment', if(a.service_feature_code = 'PED', 'Bill Payment - Education', if(a.service_feature_code = 'PLT', 'Lottery Payment', if(a.service_feature_code = 'PFT', 'Flight Payment', 'Others')))))))) as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, c.create_time as date_time, d.bp_payant_service_category_name as date_time1, c.status as status, c.account_no as sender_name from bp_service_order a, bp_request c, bp_payant_service_category d WHERE a.bp_service_order_no = c.order_no and c.bp_product_id = d.bp_payant_service_category_id and a.user_id = $userId ) union all (select a.acc_service_order_no as order_no, if(a.service_feature_code='BWO','Wallet Open',if(a.service_feature_code='BCL','Card Link',if(a.service_feature_code='BAO','Account Open','Others'))) as order_type, IFNULL(b.name , '-') as vendor, 0.00 as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(CONCAT(ifnull(c.first_name,''), ' ', ifnull(c.last_name,'')), '-') as customer_name, c.mobile as mobile_no, c.request_id as session_id, IFNULL(c.account_number , '-') as reference_no, ifnull(c.bvn,'') as comment, ifnull(d.name,'') as approver_comments, a.date_time, '' as date_time1, c.status, ifnull(c.city,'') as sender_name from acc_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join acc_request c on a.acc_service_order_no = c.order_no left join local_govt_list d on d.local_govt_id = c.local_govt_id  WHERE a.acc_service_order_no = c.order_no and a.user_id = $userId ) order by date_time desc limit 10";
					//$transaction_query = "(select a.fin_service_order_no as order_no, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)', if(a.service_feature_code = 'MP0', 'Cash-Out (Card)', if(a.service_feature_code = 'MP9', 'Fund Wallet', if(a.service_feature_code = 'COP', 'Cash-Out (Phone)', if(a.service_feature_code = 'COD', 'Cash-Out (USSD)', 'Other')))))) as order_type, if(a.service_feature_code='COP',c.account_no,IFNULL(b.name, '-')) as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, if(a.service_feature_code='COD', ifnull(c.rrn,'-'), ifnull(a.customer_name, '-')) as customer_name, if(a.service_feature_code='COD',c.account_no, a.mobile_no) as mobile_no, a.auth_code as session_id, a.reference_no, a.comment, if(c.service_feature_code='CIN',c.account_no,c.approver_comments) as approver_comments, a.date_time, '' as date_time1, c.status, c.sender_name from fin_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join fin_request c on a.fin_service_order_no = c.order_no where a.user_id = $userId ) union all (select a.e_transaction_id as order_no, if (a.service_feature_code='AFX','Recharge',if(a.service_feature_code='AFI','Fixed Recharge', 'Other')) as order_type, b.operator_description as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge+a.partner_charge) as service_charge, a.other_charge as other_charge, '-' as customer_name, a.mobile_number as mobile_no, if(a.service_feature_code='AFI',a.reference_no2, '') as session_id, a.reference_no, a.opr_plan_desc as comments, if(a.service_feature_code='AFI',a.reference_no3, '') as approver_comments, a.date_time, '' as date_time1, 'S' as status, if(a.service_feature_code='AFI', a.reference_no4, '') as sender_name from evd_transaction a, operator b where a.operator_id = b.operator_id and a.user_id = $userId ) union all (select a.bp_service_order_no as order_no, if(a.service_feature_code='PEB','Bill Payment',if(a.service_feature_code='PIT','Internet Payment', if(a.service_feature_code = 'PTV', 'Bill Payment - TV', if(a.service_feature_code='PTN','Tin Payment', if(a.service_feature_code='PFR','FRSC/Imm Payment', if(a.service_feature_code = 'PED', 'Bill Payment - Education', if(a.service_feature_code = 'PLT', 'Lottery Payment', if(a.service_feature_code = 'PFT', 'Flight Payment', 'Others')))))))) as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, c.create_time as date_time, d.bp_payant_service_category_name as date_time1, c.status as status, c.account_no as sender_name from bp_service_order a, bp_request c, bp_payant_service_category d WHERE a.bp_service_order_no = c.order_no and c.bp_product_id = d.bp_payant_service_category_id and a.service_feature_code != 'PBT' and a.user_id = $userId ) union all (select a.bp_service_order_no as order_no, 'Bill Payment - Betting' as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, c.create_time as date_time, d.bp_opay_service_provider_name as date_time1, c.status as status, c.account_no as sender_name from bp_service_order a, bp_request c, bp_opay_service_provider d WHERE a.bp_service_order_no = c.order_no and c.bp_product_id = d.bp_opay_service_provider_id and a.service_feature_code = 'PBT' and a.user_id = $userId ) union all (select a.acc_service_order_no as order_no, if(a.service_feature_code='BWO','Wallet Open',if(a.service_feature_code='BCL','Card Link',if(a.service_feature_code='BAO','Account Open','Others'))) as order_type, IFNULL(b.name , '-') as vendor, 0.00 as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(CONCAT(ifnull(c.first_name,''), ' ', ifnull(c.last_name,'')), '-') as customer_name, c.mobile as mobile_no, c.request_id as session_id, IFNULL(c.account_number , '-') as reference_no, ifnull(c.bvn,'') as comment, ifnull(d.name,'') as approver_comments, a.date_time, '' as date_time1, c.status, ifnull(c.city,'') as sender_name from acc_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join acc_request c on a.acc_service_order_no = c.order_no left join local_govt_list d on d.local_govt_id = c.local_govt_id  WHERE a.acc_service_order_no = c.order_no and a.user_id = $userId ) order by date_time desc limit 10";
					$transaction_query = "(select a.fin_service_order_no as order_no, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)', if(a.service_feature_code = 'MP0', if(c.all_in = 'Y', 'Cash-Out (Card)', 'Cash-Out (Card).'), if(a.service_feature_code = 'MP9', 'Fund Wallet', if(a.service_feature_code = 'COP', 'Cash-Out (Phone)', if(a.service_feature_code = 'COD', 'Cash-Out (USSD)', 'Other')))))) as order_type, if(a.service_feature_code='COP',c.account_no,IFNULL(b.name, '-')) as vendor, i_format(a.request_amount) as request_amount, if(c.all_in = 'Y', i_format(a.request_amount), i_format(a.total_amount)) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, if(a.service_feature_code='COD', ifnull(c.rrn,'-'), ifnull(a.customer_name, '-')) as customer_name, if(a.service_feature_code='COD',c.account_no, a.mobile_no) as mobile_no, a.auth_code as session_id, a.reference_no, c.comments as comment, if(c.service_feature_code='CIN',c.account_no,c.approver_comments) as approver_comments, a.date_time, '' as date_time1, c.status, c.sender_name from fin_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join fin_request c on a.fin_service_order_no = c.order_no where a.user_id = $userId ) union all (select a.e_transaction_id as order_no, if (a.service_feature_code='AFX','Recharge',if(a.service_feature_code='AFI','Fixed Recharge', 'Other')) as order_type, b.operator_description as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge+a.partner_charge) as service_charge, a.other_charge as other_charge, '-' as customer_name, a.mobile_number as mobile_no, if(a.service_feature_code='AFI',a.reference_no2, '') as session_id, a.reference_no, a.opr_plan_desc as comments, if(a.service_feature_code='AFI',a.reference_no3, '') as approver_comments, a.date_time, '' as date_time1, 'S' as status, if(a.service_feature_code='AFI', a.reference_no4, '') as sender_name from evd_transaction a, operator b where a.operator_id = b.operator_id and a.user_id = $userId ) union all (select a.bp_service_order_no as order_no, if(a.service_feature_code='PEB','Bill Payment',if(a.service_feature_code='PIT','Internet Payment', if(a.service_feature_code = 'PTV', 'Bill Payment - TV', if(a.service_feature_code='PTN','Tin Payment', if(a.service_feature_code='PFR','FRSC/Imm Payment', if(a.service_feature_code = 'PED', 'Bill Payment - Education', if(a.service_feature_code = 'PLT', 'Lottery Payment', if(a.service_feature_code = 'PFT', 'Flight Payment', if(a.service_feature_code = 'PBT','Bill Payment - Betting','Others'))))))))) as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, c.create_time as date_time, if(a.service_feature_code='PBT', e.bp_opay_service_provider_name, d.bp_payant_service_category_name) as date_time1, c.status as status, c.account_no as sender_name from bp_service_order a, bp_request c  left outer join bp_payant_service_category d on c.bp_product_id = d.bp_payant_service_category_id left outer join bp_opay_service_provider e on c.bp_product_id = e.bp_opay_service_provider_id WHERE a.bp_service_order_no = c.order_no and (d.bp_payant_service_category_id is not null or e.bp_opay_service_provider_id is not null) and a.user_id = $userId ) union all (select a.acc_service_order_no as order_no, if(a.service_feature_code='BWO','Wallet Open',if(a.service_feature_code='BCL','Card Link',if(a.service_feature_code='BAO','Account Open','Others'))) as order_type, IFNULL(b.name , '-') as vendor, 0.00 as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(CONCAT(ifnull(c.first_name,''), ' ', ifnull(c.last_name,'')), '-') as customer_name, c.mobile as mobile_no, c.request_id as session_id, IFNULL(c.account_number , '-') as reference_no, ifnull(c.bvn,'') as comment, ifnull(d.name,'') as approver_comments, a.date_time, '' as date_time1, c.status, ifnull(c.city,'') as sender_name from acc_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join acc_request c on a.acc_service_order_no = c.order_no left join local_govt_list d on d.local_govt_id = c.local_govt_id  WHERE a.acc_service_order_no = c.order_no and a.user_id = $userId ) order by date_time desc limit 10";
					...
					if($transaction_result) {
						...
						if (!empty($transaction_result) && mysqli_num_rows($transaction_result) > 0 ) {
							...
							while ($row = mysqli_fetch_array($transaction_result)) {
								...
								if ( $row["order_type"] == "Bill Payment" || $row["order_type"] == "Bill Payment - Education" || $row["order_type"] == "Bill Payment - TV" || $row["order_type"] == "Bill Payment - Betting") {
				...
				if ( $local_signature == $signature ) {
					...
					if($receiptType == "Recharge"){
						if ( $reportDate != "" && $orderNo != "" ) {
							$receipt_query = "select a.e_transaction_id as order_no, if (a.service_feature_code='AFX','Recharge',if(a.service_feature_code='AFI','Fixed Recharge', 'Other')) as order_type, b.operator_description as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge+a.partner_charge) as service_charge,  a.other_charge as other_charge, '-' as customer_name, a.mobile_number as mobile_no, if(a.service_feature_code='AFI',reference_no2, '') as session_id, a.reference_no, a.opr_plan_desc as comments, if(a.service_feature_code='AFI',reference_no3, '') as approver_comments, a.date_time, 'S' as status, if(a.service_feature_code='AFI', a.reference_no4, '') as sender_name from evd_transaction a, operator b where a.operator_id = b.operator_id and a.user_id = $userId and a.e_transaction_id = $orderNo and date(date_time) = '$reportDate'";
						}else {
							if ( $reportDate != "" ) {
						...
						if ( $reportDate != "" && $orderNo != "" ) {
							//$receipt_query = "select a.fin_service_order_no as order_no, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)', if(a.service_feature_code = 'MP0', 'Cash-Out (Card)', if(a.service_feature_code='COP', 'Cash-Out (Phone)', 'Other')))) as order_type, if(a.service_feature_code='COP',c.account_no,IFNULL(b.name, '-')) as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(ifnull(c.service_charge,0) + a.partner_charge) as service_charge, i_format(a.other_charge) as other_charge, ifnull(a.customer_name, '-') as customer_name, a.mobile_no, a.auth_code as session_id, a.reference_no, c.comments as comment, if(c.service_feature_code='CIN',c.account_no,c.approver_comments) as approver_comments, a.date_time, c.status, c.sender_name from fin_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join fin_request c on a.fin_service_order_no = c.order_no where a.user_id = $userId and a.fin_service_order_no = $orderNo and date(a.date_time) = '$reportDate'";
							$receipt_query = "select a.fin_service_order_no as order_no, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)', if(a.service_feature_code = 'MP0', 'Cash-Out (Card)', if(a.service_feature_code = 'MP9', 'Fund Wallet', if(a.service_feature_code = 'COP', 'Cash-Out (Phone)', if(a.service_feature_code = 'COD', 'Cash-Out (USSD)', 'Other')))))) as order_type, if(a.service_feature_code='COP',c.account_no,IFNULL(b.name, '-')) as vendor, i_format(a.request_amount) as request_amount, if (c.all_in = 'Y', i_format(a.request_amount), i_format(a.total_amount)) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, if(a.service_feature_code='COD', ifnull(c.rrn,'-'), ifnull(a.customer_name, '-')) as customer_name, if(a.service_feature_code='COD',c.account_no, a.mobile_no) as mobile_no, a.auth_code as session_id, a.reference_no, c.comments as comment, if(c.service_feature_code='CIN',c.account_no,c.approver_comments) as approver_comments, a.date_time, '' as date_time1, c.status, c.sender_name from fin_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join fin_request c on a.fin_service_order_no = c.order_no where a.user_id = $userId and a.fin_service_order_no = $orderNo and date(a.date_time) = '$reportDate'";
						}else {
							if ( $reportDate != "" ) {
								//$receipt_query = "select a.fin_service_order_no as order_no, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)', if(a.service_feature_code = 'MP0', 'Cash-Out (Card)', if(a.service_feature_code='COP', 'Cash-Out (Phone)', 'Other')))) as order_type, if(a.service_feature_code='COP',c.account_no,IFNULL(b.name, '-')) as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(ifnull(c.service_charge,0) + a.partner_charge) as service_charge, i_format(a.other_charge) as other_charge, ifnull(a.customer_name, '-') as customer_name, a.mobile_no, a.auth_code as session_id, a.reference_no, c.comments as comment, if(c.service_feature_code='CIN',c.account_no,c.approver_comments) as approver_comments, a.date_time, c.status, c.sender_name from fin_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join fin_request c on a.fin_service_order_no = c.order_no where a.user_id = $userId and date(a.date_time) = '$reportDate'";
								$receipt_query = "select a.fin_service_order_no as order_no, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)', if(a.service_feature_code = 'MP0', 'Cash-Out (Card)', if(a.service_feature_code = 'MP9', 'Fund Wallet', if(a.service_feature_code = 'COP', 'Cash-Out (Phone)', if(a.service_feature_code = 'COD', 'Cash-Out (USSD)', 'Other')))))) as order_type, if(a.service_feature_code='COP',c.account_no,IFNULL(b.name, '-')) as vendor, i_format(a.request_amount) as request_amount, if (c.all_in = 'Y', i_format(a.request_amount), i_format(a.total_amount)) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, if(a.service_feature_code='COD', ifnull(c.rrn,'-'), ifnull(a.customer_name, '-')) as customer_name, if(a.service_feature_code='COD',c.account_no, a.mobile_no) as mobile_no, a.auth_code as session_id, a.reference_no, c.comments as comment, if(c.service_feature_code='CIN',c.account_no,c.approver_comments) as approver_comments, a.date_time, '' as date_time1, c.status, c.sender_name from fin_service_order a left join bank_master b on a.bank_id = b.bank_master_id left join fin_request c on a.fin_service_order_no = c.order_no where a.user_id = $userId and date(a.date_time) = '$reportDate' order by a.date_time desc";
							}else {
						...
						if ( $reportDate != "" && $orderNo != "" ) {
							//$receipt_query = "select a.bp_service_order_no as order_no, if(a.service_feature_code='PEB','Bill Payment',if(a.service_feature_code='PIT','Internet Payment', if(a.service_feature_code = 'PTV', 'Bill Payment - TV', if(a.service_feature_code='PTN','Tin Payment', if(a.service_feature_code='PFR','FRSC/Imm Payment', if(a.service_feature_code = 'PED', 'Bill Payment - Education', if(a.service_feature_code = 'PLT', 'Lottery Payment', if(a.service_feature_code = 'PFT', 'Flight Payment', 'Others')))))))) as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, d.bp_payant_service_category_name as date_time, c.status, c.account_no as sender_name from bp_service_order a, bp_request c, bp_payant_service_category d WHERE a.bp_service_order_no = c.order_no and c.bp_product_id = d.bp_payant_service_category_id and a.user_id = $userId and a.bp_service_order_no = $orderNo and date(a.date_time) = '$reportDate'";
							$receipt_query = "select a.bp_service_order_no as order_no, if(a.service_feature_code='PEB','Bill Payment',if(a.service_feature_code='PIT','Internet Payment', if(a.service_feature_code = 'PTV', 'Bill Payment - TV', if(a.service_feature_code='PTN','Tin Payment', if(a.service_feature_code='PFR','FRSC/Imm Payment', if(a.service_feature_code = 'PED', 'Bill Payment - Education', if(a.service_feature_code = 'PLT', 'Lottery Payment', if(a.service_feature_code = 'PFT', 'Flight Payment', if(a.service_feature_code = 'PBT','Bill Payment - Betting', 'Others'))))))))) as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, if(a.service_feature_code='PBT', e.bp_opay_service_provider_name, d.bp_payant_service_category_name) as date_time, c.status, c.account_no as sender_name from bp_service_order a, bp_request c left outer join bp_payant_service_category d on c.bp_product_id = d.bp_payant_service_category_id left outer join bp_opay_service_provider e on c.bp_product_id = e.bp_opay_service_provider_id WHERE a.bp_service_order_no = c.order_no and a.user_id = $userId and (d.bp_payant_service_category_id is not null or e.bp_opay_service_provider_id is not null) and a.bp_service_order_no = $orderNo and date(a.date_time) = '$reportDate'";
						}else {
							if ( $reportDate != "" ) {
								//$receipt_query = "select a.bp_service_order_no as order_no, if(a.service_feature_code='PEB','Bill Payment',if(a.service_feature_code='PIT','Internet Payment', if(a.service_feature_code = 'PTV', 'Bill Payment - TV', if(a.service_feature_code='PTN','Tin Payment', if(a.service_feature_code='PFR','FRSC/Imm Payment', if(a.service_feature_code = 'PED', 'Bill Payment - Education', if(a.service_feature_code = 'PLT', 'Lottery Payment', if(a.service_feature_code = 'PFT', 'Flight Payment', 'Others')))))))) as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, d.bp_payant_service_category_name as date_time, c.status, c.account_no as sender_name from bp_service_order a, bp_request c, bp_payant_service_category d WHERE a.bp_service_order_no = c.order_no and c.bp_product_id = d.bp_payant_service_category_id and a.user_id = $userId and date(a.date_time) = '$reportDate'";
								$receipt_query = "select a.bp_service_order_no as order_no, if(a.service_feature_code='PEB','Bill Payment',if(a.service_feature_code='PIT','Internet Payment', if(a.service_feature_code = 'PTV', 'Bill Payment - TV', if(a.service_feature_code='PTN','Tin Payment', if(a.service_feature_code='PFR','FRSC/Imm Payment', if(a.service_feature_code = 'PED', 'Bill Payment - Education', if(a.service_feature_code = 'PLT', 'Lottery Payment', if(a.service_feature_code = 'PFT', 'Flight Payment', if(a.service_feature_code = 'PBT','Bill Payment - Betting', 'Others'))))))))) as order_type, IFNULL(c.bp_bank_code, '-') as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge, i_format(a.other_charge + a.stamp_charge) as other_charge, ifnull(c.bp_account_name, '-') as customer_name, c.bp_account_no as mobile_no, c.session_id as session_id, c.bp_transaction_id as reference_no, c.comments as comment, c.bp_bank_code as approver_comments, if(a.service_feature_code='PBT', e.bp_opay_service_provider_name, d.bp_payant_service_category_name) as date_time, c.status, c.account_no as sender_name from bp_service_order a, bp_request c left outer join bp_payant_service_category d on c.bp_product_id = d.bp_payant_service_category_id left outer join bp_opay_service_provider e on c.bp_product_id = e.bp_opay_service_provider_id WHERE a.bp_service_order_no = c.order_no and (d.bp_payant_service_category_id is not null or e.bp_opay_service_provider_id is not null) and a.user_id = $userId and date(a.date_time) = '$reportDate'";
