	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ( isset($data -> orderNo ) && !empty($data -> orderNo) &&  isset($data -> responseCode ) && !empty($data -> responseCode) 
			...
			if ( $local_signature == $signature ){	
				...
				$insert_query = "INSERT into emv_request_detail (emv_request_detail_id, transaction_id, order_no, order_type, comments, approver_comments, emv_tx_id, create_time) values (0, $emvTansactionId, $orderNo, '$orderType', '$comment', '$approverComment', '$emvTxId', now())";
