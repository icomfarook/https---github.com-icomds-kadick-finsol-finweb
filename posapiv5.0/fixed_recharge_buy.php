<?php
	...
	error_log("inside pcposapi/fixed_recharge_buy.php");
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		error_log("fixed_recharge_buy <== ".json_encode($data));

		if( isset($data->operation) && $data->operation == 'FIXED_RECHARGE_BUY') {
			error_log("inside operation == FIXED_RECHARGE_BUY method");
            		if (  isset($data->requestAmount) && !empty($data->requestAmount) && isset($data->operatorCode) && !empty($data->operatorCode)
            			...
            			&& isset($data->totalAmount) && !empty($data->totalAmount)&& isset($data->countryId) && !empty($data->countryId) 
									...
									if( $evd_trans_log_id > 0 )  {
										...
										if($evd_trans_log_result ) {
											...
											if( $evd_transaction_id > 0 ) {
												error_log("Inside EVD fixed Transaction ==> evd_transaction_id ".$evd_transaction_id);
												$acc_trans_type = 'SALE1';
												...
												$narration = "Fixed Recharge ".$operatorCode." - ".$requestedAmount;
													...
													if($evd_transaction_result ) {
														...
														if($get_acc_trans_type != "-1"){
															...
															if( $update_wallet == 0 ) {	
																...
																$data['orderNo'] = $evd_trans_log_id;
																$data['transactionId'] = $evd_trans_log_id;
															
																$url = EVD_FIXED_SERVER_URL;
																...
																if ( $curl_error == 0 ) {
																	...
																	if ( $httpcode == 200 ) {
																		...
																		if($statusCode == 0) {
																			...
																			if ( $gl_post_return_value == 0 ) {
																				error_log("Success in EVD Fixed Recharge gl_post for: ".$journal_entry_id);
																			}
																			else{
																			...
																			$response["message"] = "Your Fixed Recharge Order# $evd_transaction_id for NGN $requestedAmount submitted.";
																			$response["partnerId"] = $partnerId;
																			$response["transactionId"] = $evd_trans_log_id;
																			$response["orderNo"] = $evd_transaction_id;
																			$response["serialNo"] = $api_response['voucher']['serialNo'];
																			$response["pin"] = $api_response['voucher']['pin'];
																			$response["voucherValue"] = $api_response['voucher']['voucherValue'];
																			$response["sequenceNumber"] = $api_response['voucher']['serialNo'];
																			$response["operatorCode"] = $api_response['voucher']['operatorCode'];
																			$response["operatorId"] = $api_response['voucher']['operatorId'];
																			$response["dialString"] = $api_response['voucher']['dialString'];
																			$response["contactInfo"] = $api_response['voucher']['contactInfo'];
																			$response["dateString"] = $api_response['voucher']['dateString'];
																			...
																			if ( $update_wallet != 0 ) {
																				error_log("Error in EVD Fixed Recharge rollback_wallet for: ".$journal_entry_id);
																				insertaccountrollback($userId, $journal_entry_id, $code, $totalAmount, 2, "F", $con);
																			}else {
																				error_log("Success in EVD Fixed Recharge rollback_wallet for: ".$journal_entry_id);
																	...
																	if ( $gl_reverse_repsonse != 0 ) {
																			error_log("Error in EVD Fixed Recharge fundtransfer gl_reverse for: ".$journal_entry_id);
																		insertjournalerror($userId, $journal_entry_id, $code, "AR", "O", "N", $totalAmount, $con);
																	}else {
																		error_log("Success in process_glreverse for journal_entry_id = ".$journal_entry_id);
																	}
																...
																$msg = "Error: EVD Fixed Recharge [$narration] Order $evd_transaction_id due to failure in account update. Contact Kadick Admin.";
																...
																if ( $journal_reverse_result ) {
																	...
																	if($journal_reverse_result_code == 0) {
																		...
																		error_log("Journal Reverse is done when account balance update failed for EVD Fixed Recharge order no = $evd_transaction_id");
																	}else {
...
function maskPin($pin, $start_count, $end_count) {

	$pin_len = strlen($pin);
	$last_asterik_start = $pin_len - $end_count;
	$pin_arr = str_split($pin);
	$mask_pin = "";
	for ($x = 0; $x < $pin_len; $x++) {
		if ( $x < $start_count || $x >= $last_asterik_start) {
			$mask_pin = $mask_pin.$pin_arr[$x];			
		}else {
			$mask_pin = $mask_pin."*";
		}
	}
