				...
				if ( $local_signature == $signature ) {
					...
					if($receiptType == "Recharge"){
						if ( $reportDate != "" && $orderNo != "" ) {
							$receipt_query = "select a.e_transaction_id as order_no, if (a.service_feature_code='AFX','Recharge',if(a.service_feature_code='AFI','Fixed Recharge', 'Other')) as order_type, b.operator_description as vendor, i_format(a.request_amount) as request_amount, i_format(a.total_amount) as total_amount, i_format(a.ams_charge+a.partner_charge) as service_charge,  a.other_charge as other_charge, '-' as customer_name, a.mobile_number as mobile_no, if(a.service_feature_code='AFI',reference_no2, '') as session_id, a.reference_no, a.opr_plan_desc as comments, if(a.service_feature_code='AFI',reference_no3, '') as approver_comments, a.date_time, 'S' as status, '' as sender_name from evd_transaction a, operator b where a.operator_id = b.operator_id and a.user_id = $userId and a.e_transaction_id = $orderNo and date(date_time) = '$reportDate'";
						}else {
							if ( $reportDate != "" ) {
