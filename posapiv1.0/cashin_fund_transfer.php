	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if( isset($data->operation) && $data->operation == 'CASHIN_SUBMIT_OPERATION') {
			...
			if (  isset($data->accountNumber) && !empty($data->accountNumber) && isset($data->partnerId ) && !empty($data->partnerId)
				...
				$db_flexiRate = "N";
				$flexi_rate_query = "select state_flexi_rate_id from state_flexi_rate where state_id = 25 and (service_feature_id is null or (service_feature_id = $productId)) and active = 'Y' and (start_date is null or (start_date is not null and date(start_date) <= current_date())) and (expiry_date is null or (expiry_date is not null and date(expiry_date) >= current_date())) order by state_flexi_rate_id limit 1";
																	...
																	if ( $curl_error == 0 ) {
																		...
																		if ( $httpcode == 200 ) {
																			...
																			if($statusCode == 0) {
																				...
																				for($i = 0; $i < sizeof($serviceconfig); $i++) {
																					$cashIn_flag = insertFinanceServiceOrderComm($fin_service_order_no, $serviceconfig[$i], $journal_entry_id, $txType, $agentCharge, $con);
																					if ( $cashIn_flag == 0 ) {
