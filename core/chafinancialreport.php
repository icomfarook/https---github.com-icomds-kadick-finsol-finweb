...
<div ng-controller='chafnReportCtrl' data-ng-init="fn_load(<?php echo "'".$partyType."',"."'".$partyCode."'" ?>)">
<div class="row">
			  ...
			  <div style='text-align:center' ng-hide='isMainLoader' class="loading-spiner-holder" data-loading ><div class="loading-spiner"><img src="../common/img/gif1.gif" /></div></div>			
				<form name='finReportForm'  method='POST' novalidate>	
					<div class='row appcont' style='margin-bottom:0%'>
						<div class='col-lg-6 col-xs-12 col-sm-12 col-md-12' ng-init="creteria='all'">
						
								<label><input type='radio' ng-click='radiochange();creclick()' name='cre' ng-model='creteria' value='all'>&nbsp;<?php echo "ALL"; ?></label>								
							
								<label><input type='radio' ng-click='radiochange();creclick()' name='cre' ng-model='creteria' value='C'>&nbsp;<?php echo "Champion";  ?></label>								
						
								<label><input type='radio' ng-click='radiochange();creclick()' name='cre' ng-model='creteria' value='A'>&nbsp;<?php echo "Agent" ?></label>								
							</div>
							
							<div class='col-xs-12 col-md-12 col-lg-6 col-sm-12 ' style='text-align:right'>
								<label><input type='radio' ng-click='radiochange()' name='ba' ng-model='ba' value='ra'>&nbsp;<?php echo FINANCIAL_REPORT_MAIN_REQUEST_AMOUNT; ?></label>								
								<label><input type='radio' ng-click='radiochange()' name='ba' ng-model='ba' value='ta'>&nbsp;<?php echo FINANCIAL_REPORT_MAIN_TOTAL_AMOUNT; ?></label>								
						...
						<div class='row appcont col-lg-10 col-xs-12 col-sm-12 com-md-12' >
						
							<div class='col-lg-2 col-xs-12 col-sm-12 col-md-12'>
								<label><?php echo FINANCIAL_REPORT_MAIN_ORDER_TYPE; ?>&nbsp;<input  type='checkbox' value='Y' ng-click="impor()"  ng-model='orderdetail'/></label>
							...
							<div class='col-lg-2 col-xs-12 col-sm-12 col-md-12' >
								<label><?php echo "Champion"; ?>&nbsp;<input ng-click="impor()"  type='checkbox' value='Y'  ng-model='championDetail'/></label>
								<input ng-init= "championName=<?php echo "'".$partyCode. "-".$party_name.  "'" ?>"  readonly='true' ng-disabled="championName" id='championName'   ng-model='championName' class='form-control' name='championName' required />
								</div>	
								 
							 <div class='col-lg-2 col-xs-12 col-sm-12 col-md-12'>
								<label><?php echo "Agent"; ?>&nbsp;<input ng-click="impor()" type='checkbox'  ng-disabled="creteria==='C' || creteria==='all' " value='Y' ng-model='agentdetail'/></label>
								<select ng-init='agetnName = "ALL"' ng-disabled="creteria==='C' || creteria==='all' " ng-model='agetnName' class='form-control' name='agetnName' required>
									<option value='ALL'><?php echo STATISTICAL_REPORT_MAIN_AGENT_ALL; ?></option>
									<option ng-repeat="agent in infos"  value="{{agent.code}}">{{agent.code}} - {{agent.name}}</option>
								</select>
							</div>	
						...
						<table show-filter="true" ng-table='' class="table maintable table-bordered table-striped table-hover table-heading table-datatable" id="datatable-1">
							<thead>
								<tr> 
									...
									<th ng-show="championDetail && !agentdetail ">Champion</th>
									<th ng-show="agentdetail && championDetail" ><?php echo FINANCIAL_REPORT_MAIN_AGENT_PARENT; ?></th>
									<th ng-show="agentdetail ">Agent</th>
							...
							<tbody id='tbody'>
								 <tr ng-show='tablerow' ng-repeat="x in res">
									...
									<td ng-show="x.champion">{{ x.champion }}</td>	
									<td ng-show="x.parent">{{ x.parent }}</td>	
									<td ng-show="x.agent">{{ x.agent }}</td>
