<?php
	...
	error_log("inside pcposapiv2.0/fundwallet_charge_calculate.php");
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(isset($data -> operation) && $data -> operation == 'FUNDWALLET_CHARGE_OPERATION') {
			error_log("inside operation == FUNDWALLET_CHARGE_OPERATION method");
			...
			if ( isset($data->productId) && !empty($data->productId) && isset($data->partnerId) && !empty($data->partnerId) 
				...
				if ( $local_signature == $signature ) {
					...
					$validate_result = validateKey1($key1, $userId, $session_validity, 'J', $con);
	...
	error_log("fundwallet_charge_calculate ==> ".json_encode($response));
	echo json_encode($response);
?>