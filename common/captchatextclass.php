...
{	
	...
	{	
		/* Settings */
		$text=$this->random();
		$font = CAPTCHA_FONT_LOCATION;/* font */
		//error_log($font);
		$textColor=$this->hexToRGB($textColor);	
		...
		$lineColor = imagecolorallocate($im, 250, 5, 0);
		for( $i=0; $i<$noiceLines; $i++ ) {				
			imageline($im, mt_rand(0,$imgWidth), mt_rand(0,$imgHeight), mt_rand(0,$imgWidth), mt_rand(0,$imgHeight), $lineColor);
		}}				
