...
if(isset($_POST['login'])) {
	...
	if ( empty($_SESSION['captcha_code'] ) || strcasecmp($_SESSION['captcha_code'], $_POST['captcha_code']) != 0 ){  
		...
		$user_check_query = "SELECT user_id, user_name, password, active, COALESCE(invalid_attempt,0) as invalid_attempt, locked from user WHERE UPPER(user_name) = UPPER('".$uname."') and profile_id between 20 and 29 limit 1";
			...
			if($user_check_count > 0){
				...
				if ( $user_active == "Y" ) {
					if ( $locked != "Y" && $invalid_attempt <= $admin_attempt_limit ) {
						...
						if($successpassword) {
							$user_otp_query = "SELECT b.otp_dynamic, b.otp_value, b.key_string, b.pin, b.pin_flag FROM user a, user_otp b WHERE a.user_id = b.user_id and a.user_id = '".$user_id."' and a.active = 'Y' and a.profile_id between 20 and 29 limit 1";
							...
							if(!$user_otp_result) {
								...
								if( $formatted_otp_value == $tokensplit0 ){
									...
									if ( $pin_flag == "N" || ( $pin_flag == "Y" && $pin_sort == $tokensplit1 ) ) {
										$user_sec_check_query = "SELECT a.user_id, a.user_name, a.first_name, a.last_name, a.active, a.last_login, a.invalid_attempt, a.profile_id, a.email, a.locked, b.profile_name, b.auth_id, a.language_id as language_id from user a, profile b WHERE a.profile_id = b.profile_id and user_name = '".$uname."' and a.profile_id between 20 and 29 limit 1";
...
body {
    background: url(../common/images/bginternal.jpg) no-repeat 0px 0px !important;
