...
if($Detail == true){
    $heading = array("Agent Code","Agent Name","State","Local Government","Party Sales Type","Run Date","Available Balance","Current Balance","Advance Amount","Minimum Balance","Commission Withdrawn","Wallet Funding","Cash In","Cash Out","Evd","Bill Payment");
    $headcount = 15;
    $query = "select party_Code,party_name,state_name,local_govt_name,party_sales_type,run_date,i_format(available_balance) as available_balance,i_format(current_balance) as current_balance,i_format(advance_amount) as advance_amount,i_format(minimum_balance) as minimum_balance,i_format(comm_withdraw_amount) as comm_withdraw_amount,i_format(wallet_fund_amount) as wallet_fund_amount,i_format(cashin_amount) as cashin_amount,i_format(cashout_amount) as cashout_amount,i_format(evd_amount) as evd_amount,i_format(billpay_amount) as billpay_amount from party_wallet_history where date(run_date) = '$startDate' and party_type='A'";
}else{
    $heading = array("Party Code","Party Name","State","Local Government","Party Sales Type","Run Date","Available Balance","Current Balance","Advance Amount","Minimum Balance","Commission Withdrawn","Wallet Funding","Cash In","Cash Out","Evd","Bill Payment");
    $headcount = 15;
