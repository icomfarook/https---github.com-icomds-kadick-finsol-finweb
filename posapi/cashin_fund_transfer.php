											...
											if($fin_request_update_result) {																						
												...
												if( $fin_service_order_no > 0 ) {
													...
													if($journal_entry_id > 0) {
														...
														$fin_service_order_query = "INSERT INTO fin_service_order (fin_service_order_no, fin_trans_log_id, service_feature_code, bank_id, user_id, total_amount, request_amount, ams_charge, partner_charge, other_charge, customer_name, mobile_no, comment, date_time, service_feature_config_id, partner_id) VALUES ($fin_service_order_no, $fin_trans_log_id, 'CIN', $bankId, $userId, $totalAmount, $requestedAmount, $amsCharge, $partnerCharge, $otherCharge, '$accountName', '$mobileNo', '$narration', now(), $service_feature_config_id, $partnerId)";
																		...
																		if ( $httpcode == 200 ) {
																			...
																			if($statusCode == 0) {
																				...
																				for($i = 0; $i < sizeof($serviceconfig); $i++) {
																					$cashIn_flag = insertFinanceServiceOrderComm($fin_service_order_no, $serviceconfig[$i], $journal_entry_id, $con);
																					if ( $cashIn_flag == 0 ) {
