<?php
	...
	error_log("inside pcposapi/cashout_mpos_charge_calculate.php");
		...
		if(isset($data -> operation) && $data -> operation == 'CASHOUT_CHARGE_OPERATION') {
			...
			if ( isset($data->productId) && !empty($data->productId) && isset($data->partnerId) && !empty($data->partnerId) 
				...
				if ( $local_signature == $signature ) {
					...
					$validate_result = validateKey1($key1, $userId, $session_validity, 'H', $con);
					...
					if ($get_feature_value_result) {										
						...
						if ( substr( $db_result, 0, 1 ) === "0" ) {
                            ...
                            //$response["chargeDetail"] = '0|17|50.00|0.00|0.00#131~Kadick~5~kadickacc1~27.50,133~Champion~1001~ctestchmp101~2.25,132~Agent~1002~atestagnt102~20.25'; 
	...
	error_log("cashout_mpos_charge_calculate ==> ".json_encode($response));
	echo json_encode($response);
?>