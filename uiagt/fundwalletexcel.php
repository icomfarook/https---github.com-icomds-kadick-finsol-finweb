...
require_once   '../common/PHPExcel/Classes/PHPExcel/IOFactory.php';
	...
	$objPHPExcel = new PHPExcel();
		...
		$headcount = 6;
		$query = "select p_receipt_id, ifNull(party_code,' - ') as party_code, ifNull(payment_amount,'-') as payment_amount, ifNull(payment_approved_amount,'-') as payment_approved_amount , ifNull(payment_reference_no,'-') as payment_reference_no, ifNull(payment_date,' - ') as payment_date from payment_receipt where party_code = '".$_SESSION['party_code']."' and payment_source = 'F' and date(create_time) >= date('$startDate') and date(create_time) <= date('$endDate')";
