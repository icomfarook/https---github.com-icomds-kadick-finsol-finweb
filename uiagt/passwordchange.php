...
if(isset($_POST['submit'])) {
	...
	if ( empty($_SESSION['captcha_code'] ) || strcasecmp($_SESSION['captcha_code'], $_POST['captcha_code']) != 0 ){  
		...
		$user_check_query = "SELECT user_id,user_name, temp_password,active,invalid_attempt,locked FROM user WHERE UPPER(user_name) = UPPER('".$uname."') and profile_id in (51,52) and first_time_login = 'Y'  limit 1";
				...
				if ( $user_active == "Y" ) {
					if ( $locked != "Y" && $invalid_attempt <= $admin_attempt_limit ) {						
						...
						if($successpassword) {
							...
							$user_table_update_query = "UPDATE user SET password = '$hash_password',first_time_login = 'N'  where user_id = ".$user_id." and UPPER(user_name) = UPPER('".$username."') and profile_id in (51,52)";
