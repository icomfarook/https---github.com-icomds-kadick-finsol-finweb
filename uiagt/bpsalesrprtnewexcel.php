...
$objPHPExcel = new PHPExcel();
		...
		if($profileid  == 51) {
			$query = " SELECT a.bp_service_order_no,concat(a.service_feature_code, ' - ', d.feature_description) as service_feature_code,ifNULL(b.agent_code,'-') as agent_code,concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user ,(select name from state_list where state_id=b.state_id) as state,(select name from local_govt_list where local_govt_id = b.local_govt_id) as local,  a.request_amount,ifNULL(a.stamp_charge,'-') as stamp_charge,ifNULL(a.partner_charge,'-') as partner_charge,ifNULL(a.other_charge,'-') as other_charge, a.total_amount,a.date_time as date_time ,c.update_time,c.account_no,c.account_name,a.agent_charge,a.ams_charge, group_concat(p.charge_value ORDER BY p.service_charge_party_name) as charges FROM bp_service_order a, agent_info b, bp_request c, service_feature d,bp_service_order_comm p WHERE a.bp_service_order_no = c.order_no and c.status = 'S' and a.user_id = b.user_id and a.service_feature_code = d.feature_code and a.bp_service_order_no = p.bp_service_order_no and b.agent_code = '".$_SESSION['party_code']."'";
		}
		if($creteria == "BT") {
			if($type == "ALL") {
				$query .= " and date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no,b.agent_name,b.parent_code,c.account_no,c.account_name,d.feature_description,c.update_time,state,local,agent_code order by date_time desc ";
			}
			else{
					$query .= " and a.service_feature_code = '$type' and date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no,b.agent_name,b.parent_code,c.account_no,c.account_name,d.feature_description,c.update_time,state,local,agent_code  order by date_time desc ";
			}
		}
		if($creteria == "BO") {
			$query .= " and a.bp_service_order_no = $orderNo ggroup by a.bp_service_order_no,b.agent_name,b.parent_code,c.account_no,c.account_name,d.feature_description,c.update_time,state,local,agent_code order by a.bp_service_order_no";
		}
		if($creteria == "C") { 
			if($championCode == "ALL") {
				$query .= " and date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no,b.agent_name,b.parent_code,c.account_no,c.account_name,d.feature_description,c.update_time,state,local,agent_code order by date_time desc ";
				...
				$query .= " and b.parent_code = '$championCode' and date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no,b.agent_name,b.parent_code,c.account_no,c.account_name,d.feature_description,c.update_time,state,local,agent_code order by date_time desc ";
			}
		}
		if($creteria == "S") { 
			if($state == "ALL") {
				...
				if($local_govt_id == ""){
					$query .= " and b.state_id = '$state'  and  date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no,b.agent_name,b.parent_code,c.account_no,c.account_name,d.feature_description,c.update_time,state,local,agent_code order by date_time desc ";
				}else{
					$query .= " and b.state_id = '$state' and b.local_govt_id = '$local_govt_id'  and  date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no,b.agent_name,b.parent_code,c.account_no,c.account_name,d.feature_description,c.update_time,state,local,agent_code order by date_time desc ";
				}
		...
		error_log($query);
		$heading = array("Order No","Order Type","Agent Code","Agent Name","State","Local Government","Request Amount","Stamp Charge","Partner Charge","Other Charge","Total Amount","Date Time","Update Time",  "Account Number","Account Name","Agent Charges","Ams Charge","Total Splited Commission","Agent Commission","Champion Commission","Kadick Commission");
		$headcount = 21;
