...
require_once   '../common/PHPExcel/Classes/PHPExcel/IOFactory.php';
	...
	$objPHPExcel = new PHPExcel();
		...
		if($profileid  == 1 || $profileid  == 10 || $profileid  == 50) {
			$query = "SELECT a.fin_service_order_no, concat(a.service_feature_code, ' - ', d.feature_description) as service_feature_code,  a.request_amount, a.total_amount,  concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user, a.date_time FROM fin_service_order a, agent_info b, fin_request c, service_feature d WHERE a.fin_service_order_no = c.order_no and c.status = 'S' and a.user_id = b.user_id and a.service_feature_code = d.feature_code";
		}
		if($profileid  == 51) {
			$query = "SELECT a.fin_service_order_no, concat(a.service_feature_code, ' - ', d.feature_description) as service_feature_code,  a.request_amount, a.total_amount,  concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user, a.date_time FROM fin_service_order a, agent_info b, fin_request c, service_feature d WHERE a.fin_service_order_no = c.order_no and c.status = 'S' and a.service_feature_code = d.feature_code and b.agent_code = '".$_SESSION['party_code']."' and a.user_id = b.user_id ";
		}
		if($profileid  == 52) {
		...
		error_log($query);
		$heading = array("Order #","Order Type","Request Amount","Total Amount","Agent Name","Date Time");
		$headcount = 6;
