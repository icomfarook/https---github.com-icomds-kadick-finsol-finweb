...
$objPHPExcel = new PHPExcel();
		...
		if($profileid == 1 || $profileid == 10 || $profileid == 24 || $profileid == 22 || $profileid == 20 || $profileid == 23 || $profileid == 26 || $profileid  == 50) {
			$query = "SELECT a.fin_service_order_no,concat(a.service_feature_code, ' - ', d.feature_description) as service_feature_code,ifNULL(b.agent_code,'-') as agent_code,concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user,  a.request_amount, a.total_amount,IF(a.service_feature_code='CIN',a.auth_code, IF(a.service_feature_code='COU',a.auth_code, IF(a.service_feature_code='MP0',c.rrn,'-'))) as reference,  a.date_time as date_time ,c.update_time,ifNULL(a.agent_charge,'-') as agent_charge,ifNULL(a.ams_charge,'-') as ams_charge,ifNULL(a.stamp_charge,'-') as stamp_charge,ifNULL(a.partner_charge,'-') as partner_charge,ifNULL(a.other_charge,'-') as other_charge,ifNULL(c.service_charge,'-') as service_charge,if(c.all_in = 'Y',IFNULL(c.total_amount,0.00) - (c.request_amount),'0') as cash  FROM fin_service_order a, agent_info b, fin_request c, service_feature d,user_pos e WHERE a.fin_service_order_no = c.order_no and c.status = 'S' and a.user_id = b.user_id and a.service_feature_code = d.feature_code and a.user_id = e.user_id";
		}
		if($profileid  == 51) {
			$query = "SELECT a.fin_service_order_no,concat(a.service_feature_code, ' - ', d.feature_description) as service_feature_code,ifNULL(b.agent_code,'-') as agent_code,concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user,  a.request_amount, a.total_amount,IF(a.service_feature_code='CIN',a.auth_code, IF(a.service_feature_code='COU',a.auth_code, IF(a.service_feature_code='MP0',c.rrn,'-'))) as reference,  a.date_time as date_time ,c.update_time,ifNULL(a.agent_charge,'-') as agent_charge,ifNULL(a.ams_charge,'-') as ams_charge,ifNULL(a.stamp_charge,'-') as stamp_charge,ifNULL(a.partner_charge,'-') as partner_charge,ifNULL(a.other_charge,'-') as other_charge,ifNULL(c.service_charge,'-') as service_charge,if(c.all_in = 'Y',IFNULL(c.total_amount,0.00) - (c.request_amount),'0') as cash  FROM fin_service_order a, agent_info b, fin_request c, service_feature d,user_pos e WHERE a.fin_service_order_no = c.order_no and c.status = 'S' and a.user_id = b.user_id and a.service_feature_code = d.feature_code and a.user_id = e.user_id and b.agent_code = '".$_SESSION['party_code']."'";
		}
		if($profileid  == 52) {
	...
	if($creteria == "BT") {
		...
		error_log($query);
		$heading = array("Order No","Order Type","Agent Code","Agent","Request Amount","Total Amount","Reference", "Date Time","Update Time","Agent Charge","Ams Charge","Stamp Charge","Partner Charge","Other Charge","Total Charge","Cash");
		$headcount = 15;
