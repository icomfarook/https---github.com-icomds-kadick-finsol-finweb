...
if(isset($_POST['login'])) {
	...
	if ( empty($_SESSION['captcha_code'] ) || strcasecmp($_SESSION['captcha_code'], $_POST['captcha_code']) != 0 ){  
		...
		$user_check_query = "SELECT user_id, user_name, password, active, COALESCE(invalid_attempt,0) as invalid_attempt, locked from user WHERE UPPER(user_name) = UPPER('".$uname."') and profile_id = 10 limit 1";
			...
			if($user_check_count > 0){
				...
				if ( $user_active == "Y" ) {
					if ( $locked != "Y" && $invalid_attempt <= $admin_attempt_limit ) {
						...
						if($successpassword) {
							$user_otp_query = "SELECT b.otp_dynamic, b.otp_value, b.key_string, b.pin, b.pin_flag FROM user a, user_otp b WHERE a.user_id = b.user_id and a.user_id = '".$user_id."' and a.active = 'Y' and a.profile_id = 10 limit 1";
							...
							if(!$user_otp_result) {
								...
								if( $formatted_otp_value == $tokensplit0 ){
									...
									if ( $pin_flag == "N" || ( $pin_flag == "Y" && $pin_sort == $tokensplit1 ) ) {
										$user_sec_check_query = "SELECT a.user_id, a.user_name, a.first_name, a.last_name, a.active, a.email, ifNull(a.access_restrict,'NULL'), a.last_login, a.pos_access, a.invalid_attempt, a.profile_id, a.locked, b.profile_name, c.country_description, b.auth_id, ifnull(a.language_id, 1) as language_id from user a, profile b, country c WHERE a.profile_id = b.profile_id and c.country_id=a.country_id and  user_name = '".$uname."' and a.profile_id = 10 limit 1";		 
										...
										if ( $user_sec_check_result ) {
											...
											if($user_check_count > 0) {
												...
												$_SESSION['country_description'] =$country_description;
												$_SESSION['email'] =$email;
												$_SESSION['pos_access']	=$pos_access;
												$_SESSION['auth_id'] =$auth_id;
												$_SESSION['lanugage_id'] =$language_id;
												$_SESSION['country_id'] =ADMIN_COUNTRY_ID;
												$_SESSION['state_id'] =ADMIN_STATE_ID;
												$_SESSION['local_govt_id'] =ADMIN_LOCAL_GOVT_ID;
												$_SESSION['lang'] ="eng";
												if ( $access_restrict == "NULL" ) {
													$_SESSION['access_restrict'] = $access_restrict;
												}	
