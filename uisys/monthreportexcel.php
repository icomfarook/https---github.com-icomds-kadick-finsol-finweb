...
$msg = "Monthly  Transaction Report For  $MonthName ";
$objPHPExcel = new PHPExcel();

           if($Detail == true){
            $heading = array("Type",$MonthName);
            $headcount = 3;
            $query1 = "select 'Cashin' as type, format(ifNULL(sum(cashin_count),0),0) as count, format(ifNULL(sum(cashin_amount),0),2) as value from party_wallet_history where  date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL select 'Cashout' as type, format(ifNULL(sum(cashout_count),0),0) as count, format(ifNULL(sum(cashout_amount),0),2) as value from party_wallet_history where  date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL  select 'Billpayemnt' as type, format(ifNULL(sum(billpay_count),0),0) as count, format(ifNULL(sum(billpay_amount),0),2) as value from party_wallet_history where  date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL select 'Airtime Recharge' as type, format(ifNULL(sum(evd_count),0),0) as count, format(ifNULL(sum(evd_amount),0),2) as value from party_wallet_history where  date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL select 'Total' as type, format(sum(ifnull(cashin_count,0))+ sum(ifnull(cashout_count,0))+sum(ifnull(evd_count,0)) + sum(ifnull(billpay_count,0)),0) as count, format(sum(ifnull(cashin_amount,0)) + sum(ifnull(cashout_amount,0)) + sum(ifnull(billpay_amount,0)) + sum(ifnull(evd_count,0)),2) as value from party_wallet_history where  date_format(run_date,'%Y-%m') = '$MonthDate'";
            error_log("EXcelQuery".$query1);

            $headcount2 = 7;
            $query2 = "(select r.name as regions, format(sum(ifnull(h.evd_count,0)),0) as airtime_count , format(sum(ifnull(h.evd_amount,0)),2) as airtime_value, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as cash_count, format(sum(ifnull(h.cashin_amount,0)) + sum(ifnull(cashout_amount,0)),2) as cash_value, format(sum(ifnull(h.billpay_count,0)),0) as billpay_count, format(sum(ifnull(h.billpay_amount,0)),2) as billpay_value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code  and r.active = 'Y' and date_format(h.run_date,'%Y-%m') =  '$MonthDate' group by r.name order by r.name)  UNION ALL (select 'Total' as regions, format(sum(ifnull(h.evd_count,0)),0) as airtime_count , format(sum(ifnull(h.evd_amount,0)),2) as airtime_value, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as cash_count, format(sum(ifnull(h.cashin_amount,0)) + sum(ifnull(cashout_amount,0)),2) as cash_value, format(sum(ifnull(h.billpay_count,0)),0) as billpay_count, format(sum(ifnull(h.billpay_amount,0)),2) as billpay_value  from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code  and r.active ='Y' and date_format(h.run_date,'%Y-%m') = '$MonthDate' group by regions)";
             error_log("detail query2 = ".$query2);

             $query3 = "select format(count(t.party_code) / (select count(*) from agent_info where party_sales_chain_id = 10)*100,2) as transact_percentage from (select party_code from party_wallet_history where date_format(run_date,'%Y-%m') = '$MonthDate' and (cashin_count > 0 or cashout_count > 0 or evd_count > 0 or billpay_count > 0) group by party_code) as t;";
             error_log("detail query3 = ".$query3);
           }
           else{
            $heading = array("Type",$MonthName);
            $headcount = 3;
            $query1 = "select 'Cashin' as type, format(ifNULL(sum(cashin_count),0),0) as count, format(ifNULL(sum(cashin_amount),0),2) as value from party_wallet_history where  party_sales_type = 'External Agent' and date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL select 'Cashout' as type, format(ifNULL(sum(cashout_count),0),0) as count, format(ifNULL(sum(cashout_amount),0),2) as value from party_wallet_history where  party_sales_type = 'External Agent' and date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL  select 'Billpayemnt' as type, format(ifNULL(sum(billpay_count),0),0) as count, format(ifNULL(sum(billpay_amount),0),2) as value from party_wallet_history where  party_sales_type = 'External Agent' and date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL select 'Airtime Recharge' as type, format(ifNULL(sum(evd_count),0),0) as count, format(ifNULL(sum(evd_amount),0),2) as value from party_wallet_history where  party_sales_type = 'External Agent' and date_format(run_date,'%Y-%m') = '$MonthDate' UNION ALL select 'Total' as type, format(sum(ifnull(cashin_count,0))+ sum(ifnull(cashout_count,0))+sum(ifnull(evd_count,0)) + sum(ifnull(billpay_count,0)),0) as count, format(sum(ifnull(cashin_amount,0)) + sum(ifnull(cashout_amount,0)) + sum(ifnull(billpay_amount,0)) + sum(ifnull(evd_count,0)),2) as value from party_wallet_history where party_sales_type = 'External Agent'   and date_format(run_date,'%Y-%m') = '$MonthDate'";
            error_log("EXcelQuery".$query1);

            $headcount2 = 7;
            $query2 = "(select r.name as regions, format(sum(ifnull(h.evd_count,0)),0) as airtime_count , format(sum(ifnull(h.evd_amount,0)),2) as airtime_value, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as cash_count, format(sum(ifnull(h.cashin_amount,0)) + sum(ifnull(cashout_amount,0)),2) as cash_value, format(sum(ifnull(h.billpay_count,0)),0) as billpay_count, format(sum(ifnull(h.billpay_amount,0)),2) as billpay_value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code  and r.active = 'Y' and h.party_sales_type = 'External Agent' and date_format(h.run_date,'%Y-%m') =  '$MonthDate' group by r.name order by r.name)  UNION ALL (select 'Total' as regions, format(sum(ifnull(h.evd_count,0)),0) as airtime_count , format(sum(ifnull(h.evd_amount,0)),2) as airtime_value, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as cash_count, format(sum(ifnull(h.cashin_amount,0)) + sum(ifnull(cashout_amount,0)),2) as cash_value, format(sum(ifnull(h.billpay_count,0)),0) as billpay_count, format(sum(ifnull(h.billpay_amount,0)),2) as billpay_value  from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code  and r.active ='Y'  and h.party_sales_type = 'External Agent' and date_format(h.run_date,'%Y-%m') = '$MonthDate' group by regions)";


             error_log("detail query2 = ".$query2);
             $query3 = "select format(count(t.party_code) / (select count(*) from agent_info where party_sales_chain_id = 10)*100,2) as transact_percentage from (select party_code from party_wallet_history where  party_sales_type = 'External Agent'  and date_format(run_date,'%Y-%m') = '$MonthDate' and (cashin_count > 0 or cashout_count > 0 or evd_count > 0 or billpay_count > 0) group by party_code) as t;";
             error_log("detail query3 = ".$query3);
           }
	...
	function heading($heading,$objPHPExcel,$column) {
       ...
       while ($row = mysqli_fetch_array($result2))	{
              ...
              generateExcel2 ($secondTable, $row,$objPHPExcel,$headcount2);
        $secondTable++;
