<?php
	...
	if($action == "query") {
		...
		error_log("upgrade_detail_query == ".$upgrade_detail_query);
	...
	if($action == "userchk") {
		...
		if($application_id){
		 ...
		 error_log("application_query == ".$app_query);
				...
				if($loc_latitude == 'undefined' || $loc_latitude == "") {
					$loc_latitude = 'NULL';
				}else{
					$loc_latitude = "'".$row['loc_latitude']."'";
				}
				if($loc_longitude == 'undefined' || $loc_longitude == "") {
					$loc_longitude = 'NULL';
				}else{
					$loc_longitude = "'".$row['loc_longitude']."'";
				}
		...
		if($app_result) {
						...
						//if($select_pre_app_result){
							...
							if($select_count_attachment > 0){
								$get_sequence_number_query = "SELECT get_sequence_num(2100) as pre_seq_no";
							...
							$seq_no_id = $get_sequence_num_row['pre_seq_no'];
							$application_main_query = "";
							$comments = "Child Account for Reg  #".$application_id;
							$create_user = $_SESSION['user_id'];
							
							$pre_application_query = "INSERT INTO pre_application_info (pre_application_info_id, country_id, bvn,dob,gender, outlet_name, business_type,tax_number, address1, address2, local_govt_id, state_id, mobile_no, work_no, email, language_id, contact_person_name, contact_person_mobile, loc_latitude, loc_longitude, comments, status, create_user, create_time) VALUES ($seq_no_id, $country_id,'$bvn','$dob','$gender', '$outlet_name',$business_type, '$tax_number', '$address1', '$address2', $local_govt_id, $state_id, '$mobile_no', '$work_no', '$email', $language_id, '$cname', '$cmobile','$loc_latitude', '$loc_longitude', '$comments','E', $create_user, now())";
						...
						$select_attachment_pre_I_result = mysqli_query($con,$select_attachment_I_pre_query);
						 ...
						 $insert_pre_attachment_query="INSERT INTO pre_application_attachment (pre_application_attachment_id, pre_application_info_id, attachment_name, attachment_type, attachment_content,file) VALUES (0, $seq_no_id, '$attachment_name','$attachment_type','$attachment_content','I')";
						...
						$select_attachment_C_result = mysqli_query($con,$select_attachment_C_pre_query);
						 ...
						 $attachment_type = $row['attachment_type'];
						 $attachment_content = $row['attachment_content'];
						 $insert_pre_attachment_query = "INSERT INTO pre_application_attachment (pre_application_attachment_id, pre_application_info_id, attachment_name, attachment_type, attachment_content,file) VALUES (0, $seq_no_id, '$attachment_name','$attachment_type','$attachment_content','C')";
						 $insert_pre_result= mysqli_query($con,$insert_pre_attachment_query);
						 error_log("insert_pre_attachment_query ==".$insert_pre_attachment_query);
							}
						...
						if(!$get_sequence_number_result) {
							...
							error_log("select_agent_info_query == ".$select_agent_info_query);
						...
						$select_attachment_C_result=mysqli_query($con,$select_attachment_C_query);
						 ...
						 $update_query = "UPDATE pre_application_info SET status = 'T', application_id = $application_seq_id, update_user = $create_user, update_time = now() WHERE pre_application_info_id = $seq_no_id";
							...
							if($approve == 0) {
								    $service_query="select service_group_id,party_code,party_type from user_service_type where party_code= '$agentCode'";
									error_log("service_query == ".$service_query);
									$service_result = mysqli_query($con,$service_query);									
									
									while ($row = mysqli_fetch_assoc($service_result)) {
										error_log("service". $row['service_group_id']);
										$service = $row['service_group_id'];
									...
									$commwalletentry = commwalletentry($application_seq_id, $partycode, $type, $create_user, $creditLimit, $dailyLimit, $advanceAmount,$minimumBalance, $con);
									...
									$response["msg"] = "Child is Successfully Approved. Please Proceed Application Authorize to Complete It.";
									$response["responseCode"] = 400;
									$response["errorResponseDescription"] = mysqli_error($con);
									}else{
										$response = array();
									$response["result"] = "Success";
									$response["msg"] = "Error in while Inserting in Wallet  Table";
									$response["responseCode"] = 400;
									$response["errorResponseDescription"] = mysqli_error($con);
									}
							}else{
							$response = array();
							$response["result"] = "Success";
							$response["msg"] = "Error in while Inserting in  Agent info  Table";
							$response["responseCode"] = 400;
							$response["errorResponseDescription"] = mysqli_error($con);
							}
			...
			$response = array();
			$response["result"] = "Error";
			$response["msg"] = "Error in Getting ID of Parent";
