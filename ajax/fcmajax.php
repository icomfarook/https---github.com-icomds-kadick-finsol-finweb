	...
	if($action == "query") {
		...
		if($creteria == 'A'){
			 if (in_array("ALL", $agents))
			  {
				$query = "SELECT a.agent_name FROM agent_info a, installed_user b WHERE a.user_id = b.user_id AND status = 'L'";
				
			  }else{
				$str_of_agents = implode(',', $agents);

				$query = "SELECT a.agent_name FROM agent_info a, installed_user b WHERE a.user_id = b.user_id AND status = 'L' AND agent_code IN ('$str_of_agents')";
			  }
			
		}else if($creteria == 'UT'){
		
			if($user_type == 'ALL'){
				$query = "SELECT installed_user_id FROM installed_user";
			}else{
				$query = "SELECT installed_user_id FROM installed_user WHERE status = '$user_type'";
			}			
		}else if($creteria == 'STATE'){
			$query = "SELECT installed_user_id FROM installed_user a, agent_info b WHERE a.user_id = b.user_id AND status = 'L' AND state_id = $state";
		}else if($creteria == 'LOCAL_GOVT'){
			$query = "SELECT installed_user_id FROM installed_user a, agent_info b WHERE a.user_id = b.user_id AND status = 'L' AND local_govt_id  = $local_govt_id";
		}
		...
		if($creteria == 'A'){
			if (in_array("ALL", $agents)) {
				$query = "SELECT b.firebase_token FROM agent_info a, installed_user b WHERE a.user_id = b.user_id AND status = 'L'";
			}else{
				$str_of_agents = implode(',', $agents);
				$query = "SELECT b.firebase_token FROM agent_info a, installed_user b WHERE a.user_id = b.user_id AND status = 'L' AND agent_code IN ('$str_of_agents')";
			}
			$result =  mysqli_query($con,$query);
			$count = mysqli_num_rows($result);
			...
			if($user_type == 'ALL'){
				$query = "SELECT firebase_token FROM installed_user";
				error_log($query);
				$result =  mysqli_query($con,$query);
				$count = mysqli_num_rows($result);
				error_log("No. of agents : ".$count);
				...
				$query = "SELECT firebase_token FROM installed_user WHERE status = '$user_type'";
				error_log($query);
				$result =  mysqli_query($con,$query);
				$count = mysqli_num_rows($result);
				error_log("No. of agents : ".$count);
			...
			$query = "SELECT installed_user_topic_id FROM installed_user_topic a, agent_info b WHERE a.user_id = b.user_id AND b.state_id = $state";
			error_log($query);
			$result =  mysqli_query($con,$query);
			$count = mysqli_num_rows($result);
			error_log("No. of agents : ".$count);
			...
			$query = "SELECT installed_user_topic_id FROM installed_user_topic a, agent_info b WHERE a.user_id = b.user_id AND b.local_govt_id = $local_govt_id";
			error_log($query);
			$result =  mysqli_query($con,$query);
			$count = mysqli_num_rows($result);
			error_log("No. of agents : ".$count);
