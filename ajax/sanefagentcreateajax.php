<?php
	...
	if($action == "query") {
		...
		if($agent_select_count > 0) {
			...
			while ($row = mysqli_fetch_array($agent_select_result)) {
				...
				$data[] = array("state"=>$row['state_id'],"state"=>$row['state_id'],"pin"=>$row['pos_pin'],"mobile"=>$row['contact_person_mobile'],"country"=>$row['country_id'],"agentCode"=>$row['agentCode'],"firstName"=>$firstName,"lastName"=>$lastName,"userName"=>$row['login_name']
								...
								,"locLongitude"=>$row['loc_longitude'],"dob"=>$row['dob'],"businessTypeDesc"=>$business_type_desc);           
			}
		...
		set_time_limit(120);
		error_log("sanef agent create submit <== ".json_encode($data));
		$response = array();
		$current_time = date('Y-m-d H:i:s');
		$response['processingStartTime'] = $current_time;

		$service_feature_code = 'SAC';							
		$sanefreqsend = array();					
		...
		$stateId = mysqli_real_escape_string($con,$data->state);
		...
		$agentBusines = mysqli_real_escape_string($con,$data->businessType);	
		$gender = mysqli_real_escape_string($con,$data->gender);
		$email = mysqli_real_escape_string($con,$data->email);
		$bvn = mysqli_real_escape_string($con,$data->bvn);
		...
		if ( $sanef_seq_no > 0 ) {
			...
			//$sanefreqsend['phoneNumber2'] = "";
			$sanefreqsend['agentAddress'] = $agentAddress;
			$sanefreqsend['businessAddress'] = $agentAddress;
			$sanefreqsend['closestLandMark'] = "";
			$sanefreqsend['emailAddress'] = $email;
			$sanefreqsend['bankVerififcationNumber'] = $bvn;
			//$sanefreqsend['taxIdentififcationNumber'] = "";
			...
			if($acc_trans_log_id > 0) {
				...
				if ( $acc_trans_log_result ) {
					...
					if ( $sanef_select_result ) {
						...
						$url = SANEFAPI_SERVER_CREATE_AGENT;
						$tsec = time();
						...
						if ( $curl_error == 0 ) {
							...
							if ( $httpcode == 200 ) {
								...
								if($statusCode === 0) {
									...
									$response["message"] = "Agent [".$agentCode."] created Successfully. Sanef Agent Code: ".$api_response['agentCode'];
									$response["partnerId"] = $partnerId;
								}else {
									error_log("inside statusCode != 0");
									$update_query = "UPDATE agent_sanef_detail SET status = 'E', update_time = now() WHERE agent_code  = '$agentCode'";
									error_log("update_query = ".$update_query);
									$update_query_result = mysqli_query($con, $update_query);
									...
									$response["message"] = "Agent [".$agentCode."] not created.";
									$response["partnerId"] = $partnerId;
								}
							}else {
								error_log("inside httpcode != 200");
				...
				$response["result"] = "Error";
				$response["message"] = "DBError in trans log create";
				$response["partnerId"] = $partnerId;
			}
		}else {
			$response["agentCode"] = "";
