<?php
	...
	if($action == "calculate") {
		...
		$data = array();
		$get_feature_value_new_query = "SELECT get_feature_value_new($countryId, $stateId, $localGvtid, $productId, $partnerId,0, '$txtType', $partyCount, null, null, $user, -1) as result";
		error_log("get_feature_value_new_query = ".$get_feature_value_new_query);
		$get_feature_value_result = mysqli_query($con, $get_feature_value_new_query);
		...
		if(!$seq_no_result) {
			...
			$seq_no = $seq_no_row['seq_no'];	
			  error_log("seq_no".$seq_no);
			    if($creteria == "B") {
				   $query = "INSERT INTO comm_payout_request (comm_payout_request_id, party_type, party_code, bank_id,payout_type, comm_payout_amount, processing_amount, comm_total_amount, status, create_user, create_time) VALUES($seq_no, '$partyType','$partyCode','$bankaccount','$creteria', $paycomamt,  $procharge, $totalpaycom, 'P',$user_id,now())";
					error_log("queyr".$query);
					$result =  mysqli_query($con,$query);
					if($result){
						 error_log("$query is success");	
						...
						if($wallet_balance_result ) {
							error_log("$wallet_balance_query is success");	
                            ...
                            if($journal_entry_id1 > 0) {
								...
								if($get_acc_trans_type1 != "-1"){
                                    ...
                                    if($update_wallet1 == 0) {
											$journal_entry_id2 = process_glentry($acc_trans_type2, $seq_no, $partyCode, $partyType, $parentCode, $parentType, $to_comment, $totalpaycom, $user_id, $con);
										...
										if($journal_entry_id2 > 0) {
											...
											$update_wallet2 = walletupdateWithTransaction($acc_trans_type2, $cb_factor2, $partyType, $partyCode, $totalpaycom, $con, $user_id, $journal_entry_id2);
												...
												$response["errorResponseDescription"] = "Payout Bank Cash Transferred processed Successfully.";
												}
											}else{
											$response = array();
