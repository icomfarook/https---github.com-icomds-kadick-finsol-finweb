 <?php
	...
	if($action == "getreport") {
		...
		if($creteria == "BT") {
			if($type == "ALL") {
				$query .= " and date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no order by date_time desc ";
			}
			else{ 
		...
		if($creteria == "C") { 
			if($championCode == "ALL") {
				...
				$query .= " and b.parent_code = '$championCode' and date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no order by date_time desc ";
			}
		}
		if($creteria == "S") { 
			if($state == "ALL") {
				...
				$query .= " and b.state_id = '$state' and date(date_time) >= '$startDate' and  date(date_time) <= '$endDate' group by a.bp_service_order_no order by date_time desc ";
			}
		}
		...
		$code	=  $data->code;
		 if($profileid == 1 || $profileid == 10 || $profileid == 24 || $profileid == 22 || $profileid == 20 || $profileid == 23 || $profileid == 26  || $profileid  == 50) {
			if($code =='PEB - Electricity Payment' || $code =='PED - Education Payment'){
		    		$query = "SELECT a.bp_service_order_no,ifNULL(g.bp_trans_log_id1,'-') as  bp_trans_log_id1 ,ifNULL(g.bp_trans_log_id2,'-') as  bp_trans_log_id2,ifNULL(g.bp_trans_log_id3,'-') as  bp_trans_log_id3,concat(f.agent_name,'-','[',f.agent_code,']')as Agent_code ,a.service_feature_code, (a.total_amount) as total_amount,(a.request_amount) as  request_amount, (a.ams_charge) as ams_charge, a.partner_charge, a.other_charge,  g.mobile_no,  g.session_id,IFNULL(g.comments,'-') as comments,ifNULL(g.approver_comments,'-') as approver_comments, a.date_time,   g.bp_transaction_id,g.payment_fee,a.agent_charge,a.stamp_charge,if(a.post_status='Y','Y - Yes',if(a.post_status='E','E-Error',if(a.post_status='O','O-others','-'))) as post_status,a.post_time, concat(b.user_name,' (',b.first_name,' - ', b.last_name,') ') as user, concat(e.partner_id,' - ',e.partner_name) as partner ,concat(z.bp_biller_id,' - ',z.bp_biller_name) as Biller , ifNUll(concat(i.bp_product_id,' - ',i.bp_product_name),'-') as  Product, if(g.status='I','I-Inprogress',if(g.status='S','SUCCESS',if(g.status='E','E-Error',if(g.status='T','T-Timeout',if(g.status='C','C-Cash In',if(g.status='I','I-Inprogress',if(g.status='V','V-Validate',if(g.status='P','P-Payment Notify',if(g.status='O','O-others','-'))))))))) as status,g.account_no,ifNULL(g.account_name,'-') as account_name,g.bp_account_no,g.bp_account_name,IFNULL(g.bp_bank_code,'-') as bp_bank_code,g.create_time, (a.ams_charge + a.partner_charge + a.agent_charge) as service_charge,ifNULL(j.bp_payant_service_category_name,'-') as date_time1  ,ifNULL(j.bp_payant_service_category_name,'-') name FROM  kadick_bp_biller z,  user b,  ams_partner e, agent_info f,bp_request g LEFT JOIN bp_payant_service_category j on  g.bp_product_id = j.bp_payant_service_category_id, champion_info h,bp_biller_product i RIGHT JOIN  bp_service_order a on  a.bp_product_id = i.bp_product_id,bp_opay_service m LEFT JOIN bp_request o on  m.bp_opay_service_id = o.bp_biller_id ,bp_request k, bp_payant_service l, bp_payant_service_category N WHERE k.bp_biller_id = l.bp_payant_service_id and l.bp_payant_service_id = N.bp_payant_service_id and k.bp_product_id = N.bp_payant_service_category_id and  IF(b.user_type='C',a.user_id = h.user_id, a.user_id = f.user_id)  and a.partner_id = e.partner_id and  a.user_id = b.user_id and a.bp_service_order_no = g.order_no  and a.bp_service_order_no = $orderNo LIMIT 1";
			}
			if($code =='PTV - Cable TV Payment'){
				$query = "SELECT a.bp_service_order_no,ifNULL(g.bp_trans_log_id1,'-') as  bp_trans_log_id1 ,ifNULL(g.bp_trans_log_id2,'-') as  bp_trans_log_id2,ifNULL(g.bp_trans_log_id3,'-') as  bp_trans_log_id3,concat(f.agent_name,'-','[',f.agent_code,']')as Agent_code ,a.service_feature_code, (a.total_amount) as total_amount,(a.request_amount) as  request_amount, (a.ams_charge) as ams_charge, a.partner_charge, a.other_charge,  g.mobile_no,  g.session_id,IFNULL(g.comments,'-') as comments,ifNULL(g.approver_comments,'-') as approver_comments, a.date_time,   g.bp_transaction_id,g.payment_fee,a.agent_charge,a.stamp_charge,if(a.post_status='Y','Y - Yes',if(a.post_status='E','E-Error',if(a.post_status='O','O-others','-'))) as post_status,a.post_time, concat(b.user_name,' (',b.first_name,' - ', b.last_name,') ') as user, concat(e.partner_id,' - ',e.partner_name) as partner ,concat(z.bp_biller_id,' - ',z.bp_biller_name) as Biller , ifNUll(concat(i.bp_product_id,' - ',i.bp_product_name),'-') as  Product, if(g.status='I','I-Inprogress',if(g.status='S','SUCCESS',if(g.status='E','E-Error',if(g.status='T','T-Timeout',if(g.status='C','C-Cash In',if(g.status='I','I-Inprogress',if(g.status='V','V-Validate',if(g.status='P','P-Payment Notify',if(g.status='O','O-others','-'))))))))) as status,g.account_no,ifNULL(g.account_name,'-') as account_name,g.bp_account_no,g.bp_account_name,IFNULL(g.bp_bank_code,'-') as bp_bank_code,g.create_time, (a.ams_charge + a.partner_charge + a.agent_charge) as service_charge,ifNULL(j.bp_payant_service_category_name,'-') as date_time1,m.bp_opay_service_name , ifNULL(N.name,'-') as name FROM  kadick_bp_biller z,  user b,  ams_partner e, agent_info f,bp_request g LEFT JOIN bp_payant_service_category j on  g.bp_product_id = j.bp_payant_service_category_id, champion_info h,bp_biller_product i RIGHT JOIN  bp_service_order a on  a.bp_product_id = i.bp_product_id,bp_opay_service m LEFT JOIN bp_request o on  m.bp_opay_service_id = o.bp_biller_id ,bp_request k, bp_payant_service_category l, bp_payant_service_product N where k.bp_biller_id = l.bp_payant_service_category_id and l.bp_payant_service_category_id = N.bp_payant_service_category_id and k.bp_product_id = N.bp_payant_service_product_id and IF(b.user_type='C',a.user_id = h.user_id, a.user_id = f.user_id)  and a.partner_id = e.partner_id and  a.user_id = b.user_id and a.bp_service_order_no = g.order_no and a.bp_service_order_no = $orderNo LIMIT 1";
			}
			if($code =='PBT - Betting'){
				$query = "SELECT a.bp_service_order_no,ifNULL(g.bp_trans_log_id1,'-') as  bp_trans_log_id1 ,ifNULL(g.bp_trans_log_id2,'-') as  bp_trans_log_id2,ifNULL(g.bp_trans_log_id3,'-') as  bp_trans_log_id3,concat(f.agent_name,'-','[',f.agent_code,']')as Agent_code ,a.service_feature_code, (a.total_amount) as total_amount,(a.request_amount) as  request_amount, (a.ams_charge) as ams_charge, a.partner_charge, a.other_charge,  g.mobile_no,  g.session_id,IFNULL(g.comments,'-') as comments,ifNULL(g.approver_comments,'-') as approver_comments, a.date_time,   g.bp_transaction_id,g.payment_fee,a.agent_charge,a.stamp_charge,if(a.post_status='Y','Y - Yes',if(a.post_status='E','E-Error',if(a.post_status='O','O-others','-'))) as post_status,a.post_time, concat(b.user_name,' (',b.first_name,' - ', b.last_name,') ') as user, concat(e.partner_id,' - ',e.partner_name) as partner ,concat(z.bp_biller_id,' - ',z.bp_biller_name) as Biller , ifNUll(concat(i.bp_product_id,' - ',i.bp_product_name),'-') as  Product, if(g.status='I','I-Inprogress',if(g.status='S','SUCCESS',if(g.status='E','E-Error',if(g.status='T','T-Timeout',if(g.status='C','C-Cash In',if(g.status='I','I-Inprogress',if(g.status='V','V-Validate',if(g.status='P','P-Payment Notify',if(g.status='O','O-others','-'))))))))) as status,g.account_no,ifNULL(g.account_name,'-') as account_name,g.bp_account_no,g.bp_account_name,IFNULL(g.bp_bank_code,'-') as bp_bank_code,g.create_time, (a.ams_charge + a.partner_charge + a.agent_charge) as service_charge,ifNULL(j.bp_payant_service_category_name,'-') as date_time1,m.bp_opay_service_name ,ifNULL(N.bp_opay_service_provider_name,'-') as name FROM  kadick_bp_biller z,  user b,  ams_partner e, agent_info f,bp_request g LEFT JOIN bp_payant_service_category j on  g.bp_product_id = j.bp_payant_service_category_id, champion_info h,bp_biller_product i RIGHT JOIN  bp_service_order a on  a.bp_product_id = i.bp_product_id,bp_opay_service m LEFT JOIN bp_request o on  m.bp_opay_service_id = o.bp_biller_id ,bp_request k, bp_opay_service l, bp_opay_service_provider N WHERE k.bp_biller_id = l.bp_opay_service_id and k.bp_biller_id = N.bp_opay_service_id and k.bp_product_id = N.bp_opay_service_provider_id and IF(b.user_type='C',a.user_id = h.user_id, a.user_id = f.user_id)  and a.partner_id = e.partner_id and  a.user_id = b.user_id and a.bp_service_order_no = g.order_no  and a.bp_service_order_no = $orderNo LIMIT 1";
			}
		}
