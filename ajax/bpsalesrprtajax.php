 <?php
	...
	if($action == "getreport") {
		...
		while ($row = mysqli_fetch_array($result)) {
			$data[] = array("code"=>$row['service_feature_code'],"no"=>$row['bp_service_order_no'],"reqmount"=>$row['request_amount'],"toamount"=>$row['total_amount'],"dtime"=>$row['date_time'],"user"=>$row['user'], "reference"=>$row['reference'],"account_no"=>$row['account_no']);           
		}
		...
		$code	=  $data->code;
		 if($profileid == 1 || $profileid == 10 || $profileid == 24 || $profileid == 22 || $profileid == 20 || $profileid == 23 || $profileid == 26  || $profileid  == 50) {
			if($code =='PEB - Electricity Payment' || $code =='PED - Education Payment' || $code =='PTV - Cable TV Payment' || $code =='PBT - Betting'){
		    		$query = "SELECT a.bp_service_order_no,ifNULL(g.bp_trans_log_id1,'-') as  bp_trans_log_id1 ,ifNULL(g.bp_trans_log_id2,'-') as  bp_trans_log_id2,ifNULL(g.bp_trans_log_id3,'-') as  bp_trans_log_id3,concat(f.agent_name,'-','[',f.agent_code,']')as Agent_code ,a.service_feature_code, i_format(a.total_amount) as total_amount,i_format(a.request_amount) as  request_amount, i_format(a.ams_charge) as ams_charge, a.partner_charge, a.other_charge,  g.mobile_no,  g.session_id,IFNULL(g.comments,'-') as comments,ifNULL(g.approver_comments,'-') as approver_comments, a.date_time,   g.bp_transaction_id,g.payment_fee,a.agent_charge,a.stamp_charge,if(a.post_status='Y','Y - Yes',if(a.post_status='E','E-Error',if(a.post_status='O','O-others','-'))) as post_status,a.post_time, concat(b.user_name,' (',b.first_name,' - ', b.last_name,') ') as user, concat(e.partner_id,' - ',e.partner_name) as partner ,concat(z.bp_biller_id,' - ',z.bp_biller_name) as Biller , ifNUll(concat(i.bp_product_id,' - ',i.bp_product_name),'-') as  Product, if(g.status='I','I-Inprogress',if(g.status='S','SUCCESS',if(g.status='E','E-Error',if(g.status='T','T-Timeout',if(g.status='C','C-Cash In',if(g.status='I','I-Inprogress',if(g.status='V','V-Validate',if(g.status='P','P-Payment Notify',if(g.status='O','O-others','-'))))))))) as status,g.account_no,ifNULL(g.account_name,'-') as account_name,g.bp_account_no,g.bp_account_name,IFNULL(g.bp_bank_code,'-') as bp_bank_code,g.create_time, i_format(a.ams_charge + a.partner_charge + a.agent_charge) as service_charge,ifNULL(j.bp_payant_service_category_name,'-') as date_time1,m.bp_opay_service_name FROM  kadick_bp_biller z,  user b,  ams_partner e, agent_info f,bp_request g LEFT JOIN bp_payant_service_category j on  g.bp_product_id = j.bp_payant_service_category_id, champion_info h,bp_biller_product i RIGHT JOIN  bp_service_order a on  a.bp_product_id = i.bp_product_id,bp_opay_service m LEFT JOIN bp_request o on  m.bp_opay_service_id = o.bp_biller_id WHERE IF(b.user_type='C',a.user_id = h.user_id, a.user_id = f.user_id)  and a.partner_id = e.partner_id and  a.user_id = b.user_id and a.bp_service_order_no = g.order_no  and a.bp_service_order_no =  $orderNo LIMIT 1";
			}
		}
