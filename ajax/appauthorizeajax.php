<?php
	...
	if($action == "query") {
				...
				if(sizeof($selectedServices) > 0) {
					...
					if($update == 0) {
						...
						if($user_table_entry == 0) {								
							...
							$query2 = "INSERT INTO user_pos(user_pos_id, user_id, nibss_key1, nibss_key2, nibss_server_ip, nibss_server_port, app_timeout, control_field1, control_field2, control_field3, control_field4, control_field5, control_field6, debug_flag, status, pay_min_limit, pay_max_limit, cashin_min_limit, cashin_max_limit, cashout_min_limit, cashout_max_limit, recharge_min_limit, recharge_max_limit, mpos_simulate, create_user, create_time) values(0, $userid, '$nibss_con_comp1','$nibss_con_comp2', '$nibss_con_host', '$nibss_con_port', '$nibss_con_timeout', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'U', '$payMinLimit', '$payMaxLimit', '$cashInMinLimit', '$cashInMaxLimit', '$cashOutMinLimit', '$cashOutMaxLimit', '$rechargeMinLimit', '$rechargeMaxLimit', 'N', $createuser, now())";
							...
							if (!$result2) {
								...
								if($otp_entry == 0) {
									$approve = updateinfotableentry($userid, $id, $type, $con, $createuser, $party_code, $partycatype);
									if($approve == 0) {
										$column = 'user_setup';
										$value = 'Y';
		...
		while ($row = mysqli_fetch_array($result)) {
			$data[] = array("id"=>$row['application_id'],"country"=>$row['country_name'],"name"=>$row['outlet_name'],"type"=>$row['applier_type']
							...
							,"cpm"=>$row['contact_person_mobile'],"cpn"=>$row['contact_person_name'],"comment"=>$row['comments'],"appcomment"=>$row['approver_comments'],"appdate"=>$row['approved_time'],"Latitude"=>$row['loc_latitude'],"Longitude"=>$row['loc_longitude'],"bvn"=>$row['bvn'],"dob"=>$row['dob'],"gender"=>$row['gender'],"BusinessType"=>$row['business_type']);           
		}
		//error_log("data = ".$data);
		echo json_encode($data);
		if (!$result) {
	...
	function updateinfotableentry($userid, $id, $type, $con, $createuser, $party_code, $partycatype) {
	
		if($type == "P") {			
			$update_query = "UPDATE personal_info SET active = 'Y', user_id = $userid, party_category_type_id ='$partycatype', update_user = $createuser, update_time = now() WHERE personal_code = '$party_code'";
		}else if($type == "C") {
			$update_query = "UPDATE champion_info SET active = 'Y', user_id = $userid, party_category_type_id = '$partycatype', update_user = $createuser, update_time = now() WHERE champion_code = '$party_code'";
		}else if($type == "A" || $type == "S") {				
			$update_query = "UPDATE agent_info SET active = 'Y', user_id = $userid, party_category_type_id = '$partycatype', update_user = $createuser, update_time = now() WHERE agent_code = '$party_code'";
		}
