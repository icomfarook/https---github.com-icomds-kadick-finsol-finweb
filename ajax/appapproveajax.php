...
//error_log("service". $service);
		...
		while ($row = mysqli_fetch_array($result)) {
			$data[] = array("id"=>$row['application_id'],"country"=>$row['country_name'],"name"=>$row['outlet_name'],"type"=>$row['applier_type']
							...
							,"cpm"=>$row['contact_person_mobile'],"cpn"=>$row['contact_person_name'],"entrycomments"=>$row['comments'],"code"=>$row['parent_code'],"palogin"=>$row['parentloginname'],"Latitude"=>$row['loc_latitude'],"Longitude"=>$row['loc_longitude'],"bvn"=>$row['bvn'],"dob"=>$row['dob'],"gender"=>$row['gender'],"BusinessType"=>$row['business_type']);           
		}
		echo json_encode($data);
		if (!$result) {
			echo "Error: %s\n". mysqli_error($con);
