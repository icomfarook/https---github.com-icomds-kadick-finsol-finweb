	...
	if($action =="Uploadattachment"){
		...
		if ($count >= 1){
			...
			if($result1){
				...
				echo "Error in inserting ID Document";
			}		
		}else{
			$content = mysqli_real_escape_string($con, $content);
		...
		if($count >= 1){
			if($content2 != ''){
				...
				error_log("Company doc insert query");
				$result2 = mysqli_query($con,$query2);
				if($result2){
					...
					echo "Error in inserting New Business Document";
				}
			}
		}else{
		...
		if($count >= 1){
			if($content3 != '') {
				...
				error_log("Signature doc insert query");
				$result3 = mysqli_query($con,$query3);
				if($result3){
