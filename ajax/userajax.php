<?php
	...
	if($action == "list") {			
		...
		$query = "SELECT concat(user_name, ' (', first_name,' ', last_name, ') ') as user, COALESCE(first_time_login, 'N') as first_time_login, user_id, COALESCE(locked, 'N') as locked, COALESCE(locked_time, ' - ') as locked_time FROM user WHERE user_id= ".$id;
		$result = mysqli_query($con,$query);
		$data = array();
		while ($row = mysqli_fetch_array($result)) {
