	...
	if($action == "getreport") {
		...
		if($championCode == 'ALL'){
			...
			if($state == "ALL") {
				if($active == "ALL"){
					...
					$query ="select a.agent_code, upper(a.agent_name) as agent_name, a.login_name,  b.name as state,ifNULL(concat(a.parent_code,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = a.parent_code), 'Self'),']'),'-') as  parent_code, if(a.parent_type='A','Super Agent', if(a.parent_type='C', 'Champion', if(a.parent_type='N', '-','Other'))) as parent_type, c.name as local_govt, ifnull(a.active,'-') as active, ifnull(a.block_status,'N') as block_status,a.email ,a.contact_person_mobile from agent_info a, state_list b, local_govt_list c where a.state_id = b.state_id and b.state_id = c.state_id and a.local_govt_id = c.local_govt_id and a.active = 'Y' and a.parent_code ='$championCode'";
				}
				else{
		...
		while ($row = mysqli_fetch_array($result)) {
			$data[] = array("amtype"=>$ba, "agent_code"=>$row['agent_code'],"agent_name"=>$row['agent_name'],"champion_code"=>$row['champion_code'],"champion_name"=>$row['champion_name'],
						...
						"active"=>$row['active'],"email"=>$row['email'],"contact_person_mobile"=>$row['contact_person_mobile']);           
		}
		...
		if(!$result) {
			...
			while ($row = mysqli_fetch_array($result)) {
				$data[] = array("waltype"=>$ba,"rpartytype"=>$rpartytype, "agent_code"=>$row['agent_code'],"agent_name"=>$row['agent_name'],"login_name"=>$row['login_name'],
						...
						"contact_person_mobile"=>$row['contact_person_mobile'],"terminal_id"=>$row['terminal_id'],"email"=>$row['email']);
				}
			echo json_encode($data);
		}
