    ...
    if($Detail == true){
        if($action == "list1") {	
            ...
            while ($row = mysqli_fetch_array($result2)) {
                $data[] = array("regions"=>$row['regions'],"airtime_count"=>$row['airtime_count'],"airtime_value"=>$row['airtime_value'],"cashcount"=>$row['cashcount'],"cash_value"=>$row['cash_value'],"billpay_count"=>$row['billpay_count'],"billpay_value"=>$row['billpay_value'],"cashin_count"=>$row['cashin_count'],"cashin_value"=>$row['cashin_value']);           
            }
            echo json_encode($data);
        }
        ...
        if($action == "list1") {	
           ...
           /*  $query2 = "(select r.name as regions, format(sum(ifnull(h.evd_count,0)),0) as Count, format(sum(ifnull(h.evd_amount,0)),2) as Value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code and h.party_sales_type = 'External Agent' and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name) UNION ALL (select r.name as regions, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as Count, format(sum(ifnull(h.evd_amount,0)) + sum(ifnull(cashout_amount,0)),2) as Value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code and h.party_sales_type = 'External Agent' and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name) UNION ALL (select r.name as regions, format(sum(ifnull(h.billpay_count,0)),0) as Count, format(sum(ifnull(h.billpay_amount,0)),2) as Value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code and h.party_sales_type = 'External Agent' and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name) UNION ALL (select 'Total' as regions, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as Count, format(sum(ifnull(h.evd_amount,0)) + sum(ifnull(cashout_amount,0)),2) as Value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code and h.party_sales_type = 'External Agent' and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name)"; */
              $query2 = "(select r.name as regions, format(sum(ifnull(h.evd_count,0)),0) as airtime_count, format(sum(ifnull(h.evd_amount,0)),2) as airtime_value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code  and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name) UNION ALL (select r.name as regions, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as cashcount, format(sum(ifnull(h.evd_amount,0)) + sum(ifnull(cashout_amount,0)),2) as cash_value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code  and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name) UNION ALL (select ANY_VALUE(r.name) as regions, format(sum(ifnull(h.billpay_count,0)),0) as billpay_count, format(sum(ifnull(h.billpay_amount,0)),2) as billpay_value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name) UNION ALL (select 'Total' as regions, format(sum(ifnull(h.cashin_count,0)) + sum(ifnull(h.cashout_count,0)),0) as cashin_count, format(sum(ifnull(h.evd_amount,0)) + sum(ifnull(cashout_amount,0)),2) as cashin_value from party_wallet_history h, agent_info a, state_list s, region_list r where r.region_id = s.region_id and a.state_id = s.state_id and a.agent_code = h.party_code  and r.active = 'Y'and year(h.run_date) = year(current_date()) and month(h.run_date) = month(current_date()) and date_format(h.run_date,'%Y-%m') = '$MonthAndYear' group by r.name order by r.name)";
