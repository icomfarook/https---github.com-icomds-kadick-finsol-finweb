					...
					if($typeDetail == false) {
						if($agentName == "ALL"){
							if($agentDetail == false) {
								...
								$query ="SELECT date(a.date_time) as Date, count(*) as Count,concat(d.name) as State, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent FROM bp_service_order a, agent_info b, bp_request c,state_list d WHERE b.state_id = d.state_id and c.order_no = a.bp_service_order_no and c.status = 'S' and b.state_id = '$state' and b.local_govt_id = '$local_govt_id'  and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate'  group by b.parent_code,  State  ,b.agent_name, date(a.date_time) order by b.agent_name, date(a.date_time)";
							}
						}
						else {
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count,concat(d.name) as State FROM bp_service_order a, agent_info b, bp_request c,state_list d  WHERE b.state_id = d.state_id and c.order_no = a.bp_service_order_no and c.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and b.agent_code = '$agentName' and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate'   group by  State  ,date(a.date_time) order by date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count,concat(d.name) as State, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent FROM bp_service_order a, agent_info b, bp_request c,state_list d WHERE b.state_id = d.state_id and a.bp_service_order_no = c.order_no and c.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and b.agent_code = '$agentName' and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate'  group by b.parent_code,   State  ,b.agent_name, date(a.date_time) order by b.agent_name, date(a.date_time)";
							}
						}
					}
					else {
						if($agentName == "ALL"){
							if($agentDetail == false) {
								...
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id'  and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.user_id = b.user_id  group by b.parent_code,   State  ,a.service_feature_code, agent, date(date_time) order by a.service_feature_code, agent, date(date_time)";
							}
						}
						else {
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type',concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id=e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code and b.agent_code = '$agentName' and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate'   group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and b.agent_code = '$agentName' and a.user_id = b.user_id  group by b.parent_code, State  ,a.service_feature_code, b.agent_name, date(date_time) order by a.service_feature_code,b.agent_name, date(date_time)";
							}
					...
					if($typeDetail == false) {
						if($agentName == "ALL"){
							if($agentDetail == false) {
								...
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(d.name) as State FROM bp_service_order a, agent_info b, bp_request c,state_list d WHERE b.state_id=d.state_id and a.bp_service_order_no = c.order_no and c.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type'   group by  State  ,b.agent_name, date(a.date_time),b.parent_code order by  b.agent_name, date(date_time)";
							}
						}
						else {
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count,concat(d.name) as State FROM bp_service_order a, agent_info b, bp_request c,state_list d WHERE b.state_id= d.state_id and  a.bp_service_order_no = c.order_no and c.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and b.agent_code = '$agentName' and a.user_id = b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate'  group by  State  ,date(a.date_time) order by date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(d.name) as State FROM bp_service_order a, agent_info b, bp_request c,state_list d WHERE b.state_id=d.state_id and a.bp_service_order_no = c.order_no and c.status = 'S' and b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and date(a.date_time) between '$startDate' and '$endDate' and b.agent_code = '$agentName' and a.service_feature_code = '$type' and a.user_id = b.user_id   group by  State,b.parent_code  ,b.agent_name, date(a.date_time) order by b.agent_name, date(a.date_time)";
							}
						}
					}
					else {
						if($agentName == "ALL"){
							...
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id  and a.bp_service_order_no = d.order_no and d.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code and b.agent_code = '$agentName' and a.user_id = b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate'   group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type' and b.agent_code = '$agentName' and a.user_id = b.user_id  group by b.parent_code, State  ,a.service_feature_code, b.agent_name, date(date_time) order by  a.service_feature_code,b.agent_name, date(date_time)";
							}
				...
				if($type == "ALL") {
					if($typeDetail == false) {
						...
						if($agentName == "ALL"){
							if($agentDetail == false) {
								...
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code' and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.user_id = b.user_id  group by b.parent_code, a.service_feature_code, agent, date(a.date_time),State order by a.service_feature_code, agent, date(date_time)";
							}
						}
						else {
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type',concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id=e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code' and a.service_feature_code = c.feature_code  and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate'   group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code' and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate'  and a.user_id = b.user_id  group by b.parent_code,  State  ,a.service_feature_code, b.agent_name, date(date_time) order by a.service_feature_code,b.agent_name, date(date_time)";
							}
					...
					if($typeDetail == false) {
						...
						if($agentName == "ALL"){
							...
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id  and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code'  and a.service_feature_code = c.feature_code  and a.user_id = b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate'   group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code'  and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type'  and a.user_id = b.user_id  group by b.parent_code,   State  ,a.service_feature_code, b.agent_name, date(date_time) order by  a.service_feature_code,b.agent_name, date(date_time)";
							}
				...
				if($local_govt_id == ""){
					...
					if($typeDetail == false) {
						...
						if($agentName == "ALL"){
							if($agentDetail == false) {
								...
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code'  and  e.state_id=$state  and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.user_id = b.user_id group by b.parent_code,   State  ,a.service_feature_code, agent, date(date_time) order by a.service_feature_code, agent, date(date_time)";
							}
						}
						else {
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type',concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id=e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code'  and  e.state_id=$state and a.service_feature_code = c.feature_code  and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate' group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code'  and  e.state_id=$state and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate'  and a.user_id = b.user_id group by b.parent_code, State  ,a.service_feature_code, b.agent_name, date(date_time) order by a.service_feature_code,b.agent_name, date(date_time)";
							}
					...
					if($typeDetail == false) {
						...
						if($agentName == "ALL"){
							...
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id  and a.bp_service_order_no = d.order_no and d.status = 'S'  and b.agent_code='$sesion_party_code' and  e.state_id=$state and a.service_feature_code = c.feature_code  and a.user_id = b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate' group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S'   and b.agent_code='$sesion_party_code' and  e.state_id=$state and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type'  and a.user_id = b.user_id group by b.parent_code, State  ,a.service_feature_code, b.agent_name, date(date_time) order by  a.service_feature_code,b.agent_name, date(date_time)";
							}
				...
				if($type == "ALL") {
					if($typeDetail == false) {
						...
						if($agentName == "ALL"){
							if($agentDetail == false) {
								...
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code'  and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id'  and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.user_id = b.user_id  group by b.parent_code,   State  ,a.service_feature_code, agent, date(date_time) order by a.service_feature_code, agent, date(date_time)";
							}
						}
						else {
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type',concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id=e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code  and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate'   group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and b.agent_code='$sesion_party_code' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate'  and a.user_id = b.user_id  group by b.parent_code, State  ,a.service_feature_code, b.agent_name, date(date_time) order by a.service_feature_code,b.agent_name, date(date_time)";
							}
					...
					if($typeDetail == false) {
						...
						if($agentName == "ALL"){
							...
							if($agentDetail == false) {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id  and a.bp_service_order_no = d.order_no and d.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code  and a.user_id = b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate'   group by  State  ,a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
							else {
								$query ="SELECT date(a.date_time) as Date, count(*) as Count, concat(a.service_feature_code, ' - ', c.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(e.name) as State FROM bp_service_order a, agent_info b, service_feature c, bp_request d,state_list e WHERE b.state_id = e.state_id and a.bp_service_order_no = d.order_no and d.status = 'S' and  b.state_id = '$state' and b.local_govt_id = '$local_govt_id' and a.service_feature_code = c.feature_code and date(a.date_time) between '$startDate' and '$endDate' and a.service_feature_code = '$type'  and a.user_id = b.user_id  group by b.parent_code, State  ,a.service_feature_code, b.agent_name, date(date_time) order by  a.service_feature_code,b.agent_name, date(date_time)";
							}
