 <?php
	...
	if($action == "calculate") {
		...
		try {
			$log_constant = "";
			$log = $log_constant.date('Y:m:d h.m.s')." ************ Entering in Cash-In Payment Process ***************".PHP_EOL;
			$product	=  $data->product;
			$partner	=  $data->partner;
			$bank		=  $data->bank;
			$reqamount	=  $data->reqamount;
			...
			$log .= $log_constant.date('Y:m:d h.m.s')." Input request:".json_encode($data)."".PHP_EOL;
			...
			if($reponse_feature_value == 0) {					
				$log .= $log_constant."".date('Y:m:d h.m.s')." inside reponse_feature_value success: ".PHP_EOL;
				$log .= $log_constant."".date('Y:m:d h.m.s')." checking service config id id:.......... ".PHP_EOL;
				$log .= $log_constant."".date('Y:m:d h.m.s')." Generated Service Config id id: ".$service_feature_config_id." Requested Service Config Id: ".$sedeco.PHP_EOL;
				if($service_feature_config_id == $sedeco) {
					$log .= $log_constant."".date('Y:m:d h.m.s')." inside checking service configuration success: ".PHP_EOL;
					$log .= $log_constant."".date('Y:m:d h.m.s')." checking Ams charge details:.......... ".PHP_EOL;
					$log .= $log_constant."".date('Y:m:d h.m.s')." Generated ams charge: ".$ams_charge." Requested ams charge: ".$amsCharge.PHP_EOL;
					if(floatval($ams_charge) == floatval($amsCharge)) {
						$log .= $log_constant."".date('Y:m:d h.m.s')." inside checking ams charge success: ".PHP_EOL;
						$log .= $log_constant."".date('Y:m:d h.m.s')." checking Partner charge details:.......... ".PHP_EOL;
						$log .= $log_constant."".date('Y:m:d h.m.s')." Generated Partner charge: ".$partner_charge." Requested Partner charge: ".$partnerCharge.PHP_EOL;
						if(floatval($partner_charge) == floatval($partnerCharge)) {
							$log .= $log_constant."".date('Y:m:d h.m.s')." inside checking partnerCharge charge success: ".PHP_EOL;
							$log .= $log_constant."".date('Y:m:d h.m.s')." checking Other charge details:.......... ".PHP_EOL;
							$log .= $log_constant."".date('Y:m:d h.m.s')." Generated Other charge: ".$other_charge." Requested Other charge: ".$otherCharge.PHP_EOL;
							if(floatval($other_charge) == floatval($otherCharge)) {
								$log .= $log_constant."".date('Y:m:d h.m.s')." inside checking other charge success: ".PHP_EOL;
								$log .= $log_constant."".date('Y:m:d h.m.s')." checking Service Configuration details:.......... ".PHP_EOL;
								$log .= $log_constant."".date('Y:m:d h.m.s')." Generated Service Configuration: ".$rateparties_details." Requested Service Configuration: ".$serconfig.PHP_EOL;
								if($rateparties_details == $serconfig) {
									...
									$log .= $log_constant."".date('Y:m:d h.m.s')." checking_total_amount_progress:"." request_total_amount = ".$totalAmount." checking_total_amount = ".$request_check_total_amount.PHP_EOL;
									if(floatval($request_check_total_amount) == floatval($totalAmount)) {
										$log .= $log_constant."".date('Y:m:d h.m.s')." inside checking_total_amount after success".PHP_EOL;
										//checking_total_Charge
										$log .= $log_constant."".date('Y:m:d h.m.s')." checking_total_charge_progress:"." request_total_charge = ".$totalCharge." checking_total_amount = ".$request_check_total_charge.PHP_EOL;
										
										if(floatval($request_check_total_charge) == floatval($totalCharge)) {
											$log .= $log_constant."".date('Y:m:d h.m.s')." inside checking_total_charge after success".PHP_EOL;
												...
												if ( $available_balance <= 0 || $available_balance == "" || $available_balance == null ) {
													...
													if ( floatval($totalAmount) > floatval($available_balance) ) {
														...
														if($fin_trans_log_id > 0)  {
															...
															$cashin = new cashinRequest($country, $state, $localgovt, $partner, $accountType, $exdate, $cv, $accountNo, $accountName, $narration, $transType, $mobileNumber, $requestedAmount, $totalAmount, $partnerCharge, $amsCharge, $otherCharge, $fin_trans_log_id, '', '', $cardname );
															$code = "FCHIO";
															...
															if($get_acc_trans_type != "-1") {
																...
																if($journal_entry_id > 0) {
																	...
																	if($update_wallet == 0) {					
																		$translog = $cashin->insertTransLog($con, $fin_trans_log_id, $sedeco, $userid);
																		
																		if ( $cashin->flag == 0) {
																			error_log("before cashin sendRequest");
																			$log .= $log_constant."".date('Y:m:d h.m.s')." before cashin sendRequest".PHP_EOL;								
																			$api_response_json = sendRequest($country, $state, $localgovt, $partner, $accountType, $exdate, $cv, $accountNo, $accountName, $narration, $transType, $mobileNumber, $requestedAmount, $totalAmount, $partnerCharge, $amsCharge, $otherCharge, $fin_trans_log_id);
																			$log .= $log_constant."".date('Y:m:d h.m.s')." after cashin sendRequest".PHP_EOL;
																			error_log("after cashin sendRequest");
																			...
																			$log .= $log_constant."".date('Y:m:d h.m.s')." cashin sendRequest ==> Bank Api error_code = ".$error_code.PHP_EOL;
																			$cashin->updateTransLog($con, $fin_trans_log_id, $api_response_json);
																			if ( $cashin->flag != 0 ) {
																				error_log("error in updating translog table after cashin sendRequest response");
																				$log .= $log_constant."".date('Y:m:d h.m.s')." error in updating translog table after cashin sendRequest response".PHP_EOL;
																			}
																			...
																			if($error_code == 0) {
																				...
																				$log .= $log_constant."".date('Y:m:d h.m.s')." gl_post return_value = ".$gl_post_return_value.PHP_EOL;
																				if ( $gl_post_return_value != 0 ) {
																					error_log("Error in cashin gl_post for: ".$journal_entry_id);
																					$log .= $log_constant."".date('Y:m:d h.m.s')." Error in cashin gl_post for ".$journal_entry_id.PHP_EOL;
																					...
																					$log .= $log_constant."".date('Y:m:d h.m.s')." Success in cashin gl_post for ".$journal_entry_id.PHP_EOL;
																				}
																				...
																				$cashin->insertFinanceServiceOrder('CIN', $fin_service_order_no, $fin_trans_log_id, $userid, $sedeco, $bank, $partner, $con);
																				
																				if ( $cashin->flag == 0) {
																					error_log("Success in insert into fin_service_order table for cashin order_no = ".$fin_service_order_no);
																					error_log("Before calling post_finorder for cashin order # ".$fin_service_order_no);
																					$order_post_result = post_finorder($fin_service_order_no, $con);
																					error_log("After calling post_finorder for cashin order # ".$fin_service_order_no." result = ".$order_post_result);
																					$log .= $log_constant."".date('Y:m:d h.m.s')." post_finorder for cashin order result = ".$order_post_result.PHP_EOL;
																					...
																					for($i = 0; $i < sizeof($serviceconfig); $i++) {
																						$cashin->insertFinanceServiceOrderComm($fin_service_order_no, $serviceconfig[$i], $journal_entry_id, $con);
																						if ( $cashin->flag == 0 ) {
																					...
																					//$response["responseDescription"] = "Success: Your Cash In Order for amount NGN $requestedAmount is complete. CashIn Order No is $fin_service_order_no.";
																					$response["responseDescription"] = "Success: Your Cash In Order for amount NGN $requestedAmount is complete. CashIn Order No is $fin_service_order_no. Ref #".$api_response["transactionRef"];
																					error_log("fincashinajax.php = ".json_encode($response));
																				} else {
																					error_log("Error in insert into fin_service_order table for cashin order_no = ".$fin_service_order_no);
																					...
																					$response["signature"] = $fin_service_order_no;
																					$response["responseDescription"] = "Error: Your Cash In Order for amount NGN $requestedAmount is not complete. Order No is $fin_service_order_no.";
																					error_log("fincashinajax.php = ".json_encode($response));
																				}
																			}
																			else {
																				...
																				if ( $gl_reverse_repsonse != 0 ) {
																					error_log("Error in Cashin gl_reverse for: ".$journal_entry_id);
																			...
																			$log .= $log_constant."".date('Y:m:d h.m.s')." update_wallet = ".$update_wallet.PHP_EOL;
																			if ( $update_wallet != 0 ) {
																				error_log("Error in Cashin rollback_wallet for insert transaction log error");
																				$log .= $log_constant."".date('Y:m:d h.m.s')." Error in cashin rollback_wallet for insert transaction log error".PHP_EOL;
																				//Insert into account_rollback table with failure status
																				insertaccountrollback($userid, $journal_entry_id, $code, $totalAmount, 3, "F", $con);
																			}else {
																				//Insert into account_rollback table
																				error_log("Success in Cashin rollback_wallet for insert transaction log error");
																				$log .= $log_constant."".date('Y:m:d h.m.s')." Success in cashin rollback_wallet for insert transaction log error".PHP_EOL;
																				//Insert into account_rollback table with success status
			...
			file_put_contents($location."/$fin_trans_log_id-finweb_cashin_".date("Ydmhs").'.txt', $log, FILE_APPEND);
		}		//catch exception
		catch(Exception $e) {
	...
	function sendRequest($countryId, $stateId, $localGovtId, $partnerId, $accountType, $expiryDate, $ccv, $accountNo, $accountName, $narration, $transType, $mobileNumber, $requestedAmount, $totalAmount, $partnerCharge, $amsCharge, $otherCharge, $transactionId) {
		...
		error_log("url = ".FINAPI_SERVER_CASHIN_URL);		
		...
		$ch = curl_init(FINAPI_SERVER_CASHIN_URL);
