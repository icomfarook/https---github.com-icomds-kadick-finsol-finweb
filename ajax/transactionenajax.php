 <?php
	...
	if($action == "gettransaction") {		
		...
		if(!$get_sequence_number_result) {
			...
			$reqMsg = "refNo:".$refNo.", partner: ".$partnerId;
			$query =  "INSERT INTO fin_non_trans_log (fin_non_trans_log_id, service_feature_id, message_send_time, create_user, create_time, request_message ) VALUES ($id, 5, now(), $create_user, now(), '$reqMsg')";
			error_log($query);
			...
			if (!$result) {
				...
				$res = sendRequest($partnerId, $refNo);
	...
	function sendRequest($partnerId, $refNo) {	
