				...
				if($typeDetail == false) {
					...
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							if($ba == 'ta') {
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, concat(a.service_feature_code, ' - ', d.feature_description) as 'Order Type',concat(c.name) as State FROM acc_service_order a,agent_info b, state_list c,  service_feature d WHERE a.service_feature_code = d.feature_code and a.user_id = b.user_id  and b.state_id = c.state_id  and  date(a.date_time) between '$startDate' and '$endDate' group by  State  , service_feature_code, date(a.date_time) order by service_feature_code,date(a.date_time)";
							}
							...
							if($ba == 'ta') {
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, concat(a.service_feature_code, ' - ', d.feature_description) as 'Order Type', concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as agent,concat(c.name) as State FROM acc_service_order a, agent_info b,state_list c,  service_feature d WHERE a.service_feature_code = d.feature_code and b.state_id = c.state_id  and date(a.date_time) between '$startDate' and '$endDate' and a.user_id = b.user_id group by  State,b.parent_code  , a.service_feature_code, agent, date(a.date_time) order by  a.service_feature_code, agent, date(a.date_time)";
							}
						...
						if($agentDetail == false) {
							...
							if($ba == 'ta') {
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, concat(a.service_feature_code, ' - ', d.feature_description) as 'Order Type',concat(c.name) as State  FROM acc_service_order a, agent_info b,state_list c,  service_feature d WHERE a.service_feature_code = d.feature_code and  b.state_id = c.state_id  and b.agent_code = '$agentName' and a.user_id = b.user_id and date(a.date_time) between '$startDate' and '$endDate' group by  State  , a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
				...
				if($typeDetail == false) {
					...
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							if($ba == 'ta') {
								$query ="SELECT date(date_time) as Date, SUM(total_amount) as total_amount, service_feature_code as 'Order Type',concat(c.name) as State FROM acc_service_order a,agent_info b,state_list c,  service_feature d WHERE a.service_feature_code = d.feature_code and a.user_id = b.user_id and b.state_id = c.state_id and service_feature_code = '$type' and date(date_time) between '$startDate' and '$endDate' group by   service_feature_code,State, date(date_time) order by service_feature_code,date(date_time)";
							}
						...
						if($agentDetail == false) {
							...
							if($ba == 'ta') {
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, concat(a.service_feature_code, ' - ', d.feature_description) as 'Order Type',concat(c.name) as State FROM acc_service_order a, agent_info b,state_list c,  service_feature d WHERE a.service_feature_code = d.feature_code and b.state_id = c.state_id and  b.agent_code = '$agentName' and a.user_id = b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate' group by  State  , a.service_feature_code, date(a.date_time) order by a.service_feature_code,date(a.date_time)";
							}
				...
				if($typeDetail == false) {
					...
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							if($ba == 'ta') {
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, concat(a.service_feature_code, ' - ', d.feature_description) as 'Order Type',concat(c.name) as State FROM acc_service_order a,agent_info b, state_list c,  service_feature d WHERE a.service_feature_code = d.feature_code and b.state_id = c.state_id   and  a.user_id=b.user_id and date(a.date_time) between '$startDate' and '$endDate' and c.state_id='$state'  group by  State  , service_feature_code, date(a.date_time) order by service_feature_code,date(a.date_time)";
							}
				...
				if($typeDetail == false) {
					...
					if($agentName == "ALL"){
						if($agentDetail == false) {
							...
							if($ba == 'ta') {
								$query ="SELECT date(a.date_time) as Date, SUM(a.total_amount) as total_amount, a.service_feature_code as 'Order Type',concat(c.name) as State  FROM acc_service_order a, agent_info b,state_list c,  service_feature d WHERE a.service_feature_code = d.feature_code and b.state_id = c.state_id and a.user_id=b.user_id and a.service_feature_code = '$type' and date(a.date_time) between '$startDate' and '$endDate' and c.state_id='$state'  group by  State  , service_feature_code, date(date_time) order by service_feature_code,date(date_time)";
							}
