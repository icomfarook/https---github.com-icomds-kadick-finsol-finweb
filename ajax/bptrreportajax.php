<?php
	...
	if($action == "getreport") {
		...
		if($profileid == 1 || $profileid == 10 || $profileid == 24 || $profileid == 22 || $profileid == 20 || $profileid == 23 || $profileid == 26  || $profileid  == 50) {
			$query = "SELECT concat(c.service_feature_code, ' - ', d.feature_description) as service_feature_code, ifNULL(c.order_no,'-') as order_no , c.request_amount, c.total_amount, c.create_time, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user,c.status as stat,if(c.status='I','I-Inprogress',if(c.status='S','S-Success',if(c.status='E','E-Error',if(c.status='T','T-Timeout',if(c.status='C','C-Cash In',if(c.status='V','V-Validate',if(c.status='S','S-Success',if(c.status='P','P-Payment Notify',if(c.status='G','G-Triggered',if(c.status='O','O-others','-')))))))))) as status,   c.account_no as rrn FROM agent_info b, bp_request c, service_feature d WHERE c.user_id = b.user_id and c.service_feature_code = d.feature_code ";
		}
		if($profileid  == 51) {
		...
		while ($row = mysqli_fetch_array($result)) {
			$data[] = array("code"=>$row['service_feature_code'],"no"=>$row['order_no'],"reqmount"=>$row['request_amount'],"toamount"=>$row['total_amount'],"dtime"=>$row['create_time'],"user"=>$row['user'], "status"=>$row['status'], "rrn"=>$row['rrn'],"stat"=>$row['stat']);           
		}
		...
		if($profileid == 1 || $profileid == 10 || $profileid == 24 || $profileid == 22 || $profileid == 20 || $profileid == 23 || $profileid == 26  || $profileid  == 50) {
		    	...
		    	$query = "SELECT a.order_no, ifNULL(a.bp_trans_log_id1,'-') as  bp_trans_log_id1 ,ifNULL(a.bp_trans_log_id2,'-') as  bp_trans_log_id2,ifNULL(a.bp_trans_log_id3,'-') as  bp_trans_log_id3, a.service_feature_code, a.total_amount,ifNULL(g.ams_charge,'-') as ams_charge,a.bp_transaction_id,a.request_amount, a.service_charge, a.partner_charge, a.other_charge, a.mobile_no,  a.comments, a.create_time, a.create_time,ifNULL(a.payment_fee,'-') as payment_fee,g.agent_charge,g.stamp_charge,g.post_time,if(g.post_status='Y','Y - Yes',if(g.post_status='E','E-Error',if(g.post_status='O','O-others','-'))) as post_status, a.update_time, concat(b.user_name,' (',b.first_name,' - ', b.last_name,') ') as user,concat(d.bank_master_id,' - ',d.name) as bank,concat(h.bp_biller_id,' - ',h.bp_biller_name) as Biller , ifNUll(concat(i.bp_product_id,' - ',i.bp_product_name),'-') as  Product, concat(e.partner_id,' - ',e.partner_name) as partner,  if(a.status='I','I-Inprogress',if(a.status='S','S-Success',if(a.status='E','E-Error',if(a.status='T','T-Timeout',if(a.status='C','C-Cash In',if(a.status='I','I-Inprogress',if(a.status='V','V-Validate',if(a.status='S','S-Success',if(a.status='P','P-Payment Notify',if(a.status='O','O-others','-')))))))))) as status, a.approver_comments,a.account_no,a.account_name,a.bp_account_no,a.bp_account_name,a.bp_bank_code,a.session_id FROM kadick_bp_biller h, user b, bank_master d, ams_partner e, agent_info f,bp_biller_product i RIGHT JOIN  bp_service_order z on  z.bp_product_id = i.bp_product_id,bp_request a , bp_service_order g WHERE  a.user_id = f.user_id  and g.partner_id = e.partner_id  and a.user_id = b.user_id  and a.order_no = $orderNo LIMIT 1";
