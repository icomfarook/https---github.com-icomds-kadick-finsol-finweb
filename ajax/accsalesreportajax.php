 <?php
	...
	if($action == "getreport") {
		...
		if($profileid  == 52) {
			$query = "SELECT concat(a.service_feature_code, ' - ', d.feature_description) as service_feature_code, a.acc_service_order_no, a.total_amount, a.date_time as date_time, concat(f.name) as bank,IF(a.service_feature_code='BAO','Account Open', IF(a.service_feature_code='BWL','Wallet Open', IF(a.service_feature_code='BCL','Card Link','-'))) as reference, concat(b.agent_name,' [',ifNULL((select champion_name FROM champion_info WHERE champion_code = b.parent_code), 'Self'),']') as user FROM acc_service_order a, agent_info b, acc_request c, service_feature d WHERE a.acc_service_order_no = c.order_no and c.status = 'S' and a.service_feature_code = d.feature_code and b.agent_code = '".$_SESSION['party_code']."' and b.sub_agent = 'Y' and a.user_id = b.user_id ";
		}
		if($creteria == "BT") {
			if($type == "ALL") {
		...
		$code	=  $data->code;
		 if($profileid == 1 || $profileid == 10 || $profileid == 24 || $profileid == 22 || $profileid == 20 || $profileid == 23 || $profileid == 26  || $profileid  == 50) {
			...
			if($code =='CIN - Cash In'){
				$query = " SELECT a.acc_service_order_no, a.acc_trans_log_id, a.service_feature_code, a.total_amount,a.request_amount, a.ams_charge, a.partner_charge, a.other_charge, a.customer_name, a.mobile_no, a.auth_code, a.reference_no, a.comment, a.date_time, a.post_status, a.post_time, a.service_feature_config_id, concat(b.user_name,' (',b.first_name,' - ', b.last_name,') ') as user,concat(d.bank_master_id,' - ',d.name) as bank, concat(e.partner_id,' - ',e.partner_name) as partner ,if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)',if(a.service_feature_code='MP0','Cash-Out (Card)','-'))) as transaction_type,  g.sender_name,if(g.status='S','SUCCESS',if(g.status='G','TRIGGERED','-')) as status FROM acc_service_order a, user b, bank_master d, ams_partner e, agent_info f, acc_request g WHERE a.user_id = f.user_id and f.agent_code = '".$_SESSION['party_code']."' and a.partner_id = e.partner_id and a.bank_id = d.bank_master_id and a.user_id = b.user_id and a.acc_service_order_no = g.order_no and a.acc_service_order_no = $orderNo";
			}else{
				$query = " SELECT a.acc_service_order_no, a.acc_trans_log_id, a.service_feature_code, g.total_amount,g.request_amount, a.ams_charge, g.partner_charge, g.other_charge,(g.service_charge + g.partner_charge) as service_charge, a.customer_name, a.mobile_no, a.auth_code, a.reference_no, a.comment, a.date_time, a.post_status, a.post_time, a.service_feature_config_id, concat(b.user_name,' (',b.first_name,' - ', b.last_name,') ') as user,concat(e.partner_id,' - ',e.partner_name) as partner, if(a.service_feature_code='CIN','Cash-In (Account)',if(a.service_feature_code='COU','Cash-Out (Account)',if(a.service_feature_code='MP0','Cash-Out (Card)','-'))) as transaction_type,  g.sender_name,if(g.status='S','SUCCESS',if(g.status='G','TRIGGERED','-')) as status, g.approver_comments,f.agent_code FROM acc_service_order a, user b, ams_partner e, agent_info f, acc_request g WHERE a.user_id = f.user_id and f.agent_code = '".$_SESSION['party_code']."' and a.partner_id = e.partner_id and a.user_id = b.user_id and a.acc_service_order_no = g.order_no and a.acc_service_order_no = $orderNo";
			}
		}
